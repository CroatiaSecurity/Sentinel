...
| No process memory touch until proven malicious | Opportunistic `Process.Modules` / `Process.MainModule` / `OpenProcess(PROCESS_VM_READ)` is forbidden. Those APIs self-terminate Denuvo/anti-cheat games. Use `SecurityValidation.GetProcessImagePath` (QUERY_LIMITED) for path resolution. Memory/module inspection only via `SecurityValidation.MayInspectProcessMemory(true)` after independent evidence, and never on `IsGameOrAntiCheatPath`. |
