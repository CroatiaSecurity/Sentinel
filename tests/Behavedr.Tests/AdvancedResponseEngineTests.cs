...
namespace Behavedr.Tests
{
    public class AdvancedResponseEngineTests
    {
        ...
        {
            var tempDir = Path.Combine(Path.GetTempPath(), "behavedr_are_test_" + Guid.NewGuid().ToString("N")[..8]);
        ...
        {
            var tempDir = Path.Combine(Path.GetTempPath(), "behavedr_are_test2_" + Guid.NewGuid().ToString("N")[..8]);
            Directory.CreateDirectory(tempDir);
            try
            {
                var config = new BehavedrConfig { ActiveResponse = true };
        ...
        {
            var tempDir = Path.Combine(Path.GetTempPath(), "behavedr_are_test_suppressed_" + Guid.NewGuid().ToString("N")[..8]);
            Directory.CreateDirectory(tempDir);
            try
            {
                var config = new BehavedrConfig { ActiveResponse = true };
    ...
    public class AdvancedResponseEngineExtendedTests : IDisposable
    {
        ...
        {
            _tempDir = Path.Combine(Path.GetTempPath(), "behavedr_are_ext_" + Guid.NewGuid().ToString("N")[..8]);
        ...
        public async Task HandleAsync_SelfExclusion_NeverKillsOwnProcess()
        {
            ...
            var detection = new DetectionEvent
            {
                ...
                Tier = DetectionTier.Tier1Behavioral,
                AuthorizedResponse = ResponseAction.KillProcessTree,
                ProcessName = "Behavedr.Service.exe",
        ...
        public async Task HandleAsync_RemoveRegistryEntry_Tier1_ExecutesRemoval()
        {
            var detection = new DetectionEvent
            {
                ...
                Metadata = new Dictionary<string, string>
                {
                    ...
                    { "ValueName", "FakeValue_BehavedrTest_DoesNotExist" }
                }
            };
