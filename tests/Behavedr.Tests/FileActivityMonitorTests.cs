...
namespace Behavedr.Tests
{
    public class FileActivityMonitorTests : IDisposable
    {
        ...
        {
            _tempPath = Path.Combine(Path.GetTempPath(), "behavedr_fam_test_" + Guid.NewGuid().ToString("N")[..8]);
        ...
        private FileActivityMonitor CreateMonitor(out SignerTrustService signerTrust)
        {
            ...
            var config = new BehavedrConfig { WatchPath = _tempPath };
        ...
        public void IsTrustedSystemWriter_TrustedProcessNames_ReturnsTrue()
        {
            ...
            Assert.True(monitor.IsTrustedSystemWriter(1234, "windowsbehavedr.service", "dummy.txt"));
        }
