            ...
            try
            {
                foreach (var proc in Process.GetProcesses())
                {
                    try
                    {
                        ...
                        // IMPORTANT: This runs BEFORE the signed-process bypass because signed
                        // processes are prime sideloading targets — attackers drop malicious DLLs
                        // next to signed binaries precisely because they know EDR tools trust them.
                        // A signed host with a sideloaded DLL must still be remediated.
                        ...
                        // IMPORTANT: This runs BEFORE the signed-process bypass because injected
                        // DLLs target signed processes (browsers, Discord, etc.) specifically.
                        // Without this check, UnloadInjectedDllAsync in AdvancedResponseEngine
                        // would never fire for signed hosts since no detection event is emitted.
                        int currentModuleCount = 0;
                        try { currentModuleCount = proc.Modules.Count; } catch { currentModuleCount = -1; }

                        if (currentModuleCount > 0 && _previousModuleCounts.TryGetValue(proc.Id, out int prevCount))
                        {
                        ...
                        if (currentModuleCount > 0) _previousModuleCounts[proc.Id] = currentModuleCount;

                        // Bypass memory scanner entirely for trusted processes signed by reputable publishers
                        // (but only after sideloading + injection checks — signature doesn't protect against DLL drops/injection)
