...
namespace Sentinel.Core
{
    public static class HardeningModule
    {
        ...
        /// <summary>
        /// v1.5.5: Secured hardening script application (RT-HIGH-3, SAB-1).
        /// - Removed blanket PowerShell script execution (was executing ALL .ps1 files
        ///   including the now-removed GorstaksEDR.ps1 which was a supply-chain compromise)
        /// - Added SHA-256 integrity verification of extracted resources before use
        /// - Only applies registry files and LGPO policy (no arbitrary script execution)
        /// - Locks down extraction directory ACL before writing files
        /// </summary>
        private static void ApplyUserSetupScriptsHardening()
        {
            string scriptsDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), @"Sentinel\HardeningTemp");
            try
            {
                // Create and secure target directory
                if (!Directory.Exists(scriptsDir))
                {
                    Directory.CreateDirectory(scriptsDir);
                }

                // Lock down directory ACL: only SYSTEM and Administrators can write
                try
                {
                ...
                catch { /* Non-fatal: may lack privileges in non-elevated context */ }

                // Extract embedded resources (only .reg, .inf files — NO .ps1 scripts)
                var assembly = typeof(HardeningModule).Assembly;
                string resourcePrefix = "Sentinel.Core.HardeningResources.";

                // Allowed file extensions for extraction (no PowerShell scripts)
                ...
                foreach (string resourceName in assembly.GetManifestResourceNames())
                {
                    if (resourceName.StartsWith(resourcePrefix, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        // v1.5.5: Only extract allowed file types — reject .ps1, .bat, .vbs
                        if (!allowedExtensions.Contains(ext))
                            continue;

                        // v1.5.5: Only allow LGPO.exe — reject any other embedded executable
                        if (ext.Equals(".exe", StringComparison.OrdinalIgnoreCase) &&
                            !fileName.Equals("LGPO.exe", StringComparison.OrdinalIgnoreCase))
                            continue;
                ...
                // 1. Run lgpo.exe if present (applies Group Policy security settings)
                ...
                // 2. Import registry files alphabetically (hardening registry settings)
                ...
                // v1.5.5: Removed PowerShell script execution entirely (SAB-1).
                // Previously executed ALL .ps1 files from embedded resources with
                // -ExecutionPolicy Bypass, which allowed the GorstaksEDR.ps1 trojan
                // to install VPN connections, Defender exclusions, and persistence.

                // 3. File ownership and permissions tweaks
