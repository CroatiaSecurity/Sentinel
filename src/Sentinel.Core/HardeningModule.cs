...
namespace Sentinel.Core
{
    public static class HardeningModule
    {
        ...
        public static bool ApplyOrFail()
        {
            try
            {
                // Disable FIPS Algorithm Policy to ensure non-compliant cryptography does not throw exceptions
                try
                {
                    using var key = Microsoft.Win32.Registry.LocalMachine.CreateSubKey(@"SYSTEM\CurrentControlSet\Control\Lsa\FipsAlgorithmPolicy");
                ...
                try
                {
                    using var key = Microsoft.Win32.Registry.LocalMachine.CreateSubKey(@"SYSTEM\CurrentControlSet\Control\Lsa");
                    key?.SetValue("FipsAlgorithmPolicy", 0, Microsoft.Win32.RegistryValueKind.DWord);
                }
                catch { }
        ...
        private static void ApplyUserSetupScriptsHardening()
        {
            ...
            try
            {
                ...
                // Extract all embedded resources from "Sentinel.Core.HardeningResources."
