    ...
    public sealed class RegistryMonitor : BackgroundService
    {
        ...
        private async Task EvaluateRegsvr32Async(int pid, string cmdLine)
        {
            ...
            if (confidence >= 0.75)
            {
                ...
                _toastService.ShowToast("Behavedr Alert: Suspicious regsvr32", $"PID {pid}: {cmdLine}");
            }
            else
            {
                var detection = new DetectionEvent
        ...
        private async Task EvaluateRegExeAsync(int pid, string cmdLine)
        {
            ...
            if (confidence >= 0.75)
            {
                ...
                await _detectionEngine.EmitAsync(detection);
                _toastService.ShowToast("Behavedr Alert: Suspicious reg.exe", $"PID {pid}: {cmdLine}");
            }
            else
            {
                var detection = new DetectionEvent
        ...
        private async Task EvaluateRegeditAsync(int pid, string cmdLine)
        {
            ...
            if (confidence >= 0.75)
            {
                _toastService.ShowToast("Behavedr Alert: regedit.exe detected", $"PID {pid}: {cmdLine}");
            }
        }
        ...
        private async Task EvaluateAutorunEntry(string valueName, string valueData, bool isHklm, string keyPath)
        {
            ...
            if (confidence >= 0.50)
            {
                ...
                await _detectionEngine.EmitAsync(detection);
                _toastService.ShowToast("Behavedr Alert: Autorun Entry", $"'{valueName}' added to {hive}\\Run");
            }
            else
            {
        ...
        private async Task EvaluateNewService(string serviceName, string imagePath)
        {
            ...
            if (confidence >= 0.50)
            {
                ...
                await _detectionEngine.EmitAsync(detection);
                _toastService.ShowToast("Behavedr Alert: New Service", $"'{serviceName}' registered");
            }
        }
            ...
            try
            {
                ...
                foreach (var kvp in currentClsid)
                {
                    ...
                    if (confidence >= 0.50)
                    {
                        ...
                        await _detectionEngine.EmitAsync(detection);
                        _toastService.ShowToast("Behavedr Alert: COM Hijack", $"CLSID {{{clsid}}} registered");
                    }
                }
            ...
            try
            {
                ...
                foreach (var valueName in new[] { "ProxyEnable", "ProxyServer", "AutoConfigURL" })
                {
                    ...
                    if (currentVal != baselineVal)
                    {
                        ...
                        var reasoning = $"Proxy hijacking reroutes user traffic. Behavedr detected an unauthorized modification to system proxy settings.";
                        ...
                        _toastService.ShowToast("Behavedr Alert: Proxy Hijack", $"Proxy '{valueName}' changed under {prefix}");
            ...
            try
            {
                ...
                foreach (var valueName in key.GetValueNames())
                {
                    ...
                    if (!_extensionBaseline.Contains(baselineKey))
                    {
                        ...
                        var reasoning = $"Malicious extensions can read passwords, hijack tabs, and steal sessions. Behavedr detected an unauthorized force-installed browser extension.";
                        ...
                        _ = _detectionEngine.EmitAsync(detection);
                        _toastService.ShowToast("Behavedr Alert: Browser Policy Hijack", $"New force-installed extension in {browserName}");
