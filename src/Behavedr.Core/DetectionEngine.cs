...
namespace Behavedr.Core
{
    ...
    public class DetectionEngine
    {
        ...
        private BehavedrOrchestrator? _orchestrator;

        /// <summary>
        /// Late-bound orchestrator injection to avoid circular DI.
        /// Called by BehavedrService during startup wiring.
        /// </summary>
        public void SetOrchestrator(BehavedrOrchestrator orchestrator) => _orchestrator = orchestrator;
                    ...
                    while (reader.TryRead(out var context))
                    {
                        try
                        {
                            ...
                            if (context.TriggeringEvent is ProcessTelemetry pt)
                            {
                                ...
                                // this by triggering rapid stop/start cycles to keep Behavedr down.
                                var ct = _cts.Token;
                                _ = Task.Run(async () =>
                                {
                                    try
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(imagePath) && System.IO.File.Exists(imagePath))
                                        {
                                            ...
                                            // Behavedr.Agent.exe and Behavedr.Service.exe are unsigned
        ...
        private async Task HandleDetectionEventAsync(DetectionEvent detection)
        {
            ...
            // v1.3.2: Route through BehavedrOrchestrator for incident grouping and response coordination
