...
{
    /// <summary>
    /// Service-side watchdog that keeps Behavedr.Agent.exe alive in the user's
    /// interactive session.
    ///
    ...
    public sealed class AgentWatchdog : BackgroundService
    {
        ...
        private const string AgentProcessName = "Behavedr.Agent";
        private const string AgentExeName = "Behavedr.Agent.exe";
        ...
        private async Task CheckAgentAsync(CancellationToken ct)
        {
            ...
            if (_killCount >= KillThresholdForAlert && !_alertFired)
            {
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Evidence = $"Behavedr.Agent.exe has been absent {_killCount} times in the last " +
                               $"{KillWindowDuration.TotalMinutes:F0} minutes. Watchdog is relaunching it.",
                    Reasoning = "The Behavedr user-session agent keeps dying. This may indicate an attacker " +
