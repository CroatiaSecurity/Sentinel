    ...
    public class SecureCacheStore
    {
        ...
        public SecureCacheStore(string? customPath = null)
        {
            ...
            else
            {
                ...
                _secureDir = Path.Combine(programData, "Behavedr", "Secure");
            }
        ...
        private static byte[] GenerateBootBoundKey()
        {
            ...
            {
                var programData = Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData);
                var entropyFile = Path.Combine(programData, "Behavedr", "Secure", ".install_entropy");
            ...
            ms.Write(Encoding.UTF8.GetBytes("behavedr-secure-cache-hmac-v2"));
        ...
        private static byte[] Protect(byte[] data)
        {
            ...
            try
            {
                if (CryptProtectData(ref dataIn, "BehavedrCache", ref entropy, IntPtr.Zero, ref prompt, CRYPTPROTECT_UI_FORBIDDEN, ref dataOut))
                {
