                ...
                while ((record = reader.ReadEvent()) != null)
                {
                    using (record)
                    {
                        ...
                        _ = _detectionEngine.EmitAsync(new DetectionEvent
                        {
                            ...
                            Reasoning = "Windows Defender's Attack Surface Reduction rule for LSASS credential theft was triggered. This confirms an active credential dumping attempt was blocked by Defender, and Behavedr independently corroborates the threat.",
                            Confidence = 0.95,
                            Tier = DetectionTier.Tier1Behavioral,
                            AuthorizedResponse = ResponseAction.KillProcessTree,
                            ProcessName = shortName,
