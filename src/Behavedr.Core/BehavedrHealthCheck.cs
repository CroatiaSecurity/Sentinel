...
namespace Behavedr.Core
{
    ...
    /// </summary>
    public sealed class BehavedrHealthCheck : BackgroundService
    {
        private readonly ILogger<BehavedrHealthCheck> _logger;
        private readonly JsonlEventLogger _eventLogger;
        private readonly BehavedrMetrics _metrics;
        private static readonly TimeSpan Interval = TimeSpan.FromMinutes(5);

        public BehavedrHealthCheck(
            ILogger<BehavedrHealthCheck> logger,
        ...
        {
            _logger.LogInformation("[BehavedrHealthCheck] Started (interval: {Interval})", Interval);
            ...
            do
            {
                try
                {
                    ...
                    if (workingSetMb > 500)
                        _logger.LogWarning("[BehavedrHealthCheck] High memory usage: {MB:F0}MB", workingSetMb);
                    if (handleCount > 5000)
                        _logger.LogWarning("[BehavedrHealthCheck] High handle count: {Handles}", handleCount);
                    if (logFileSizeMb > 40)
                        _logger.LogWarning("[BehavedrHealthCheck] Log file approaching rotation threshold: {MB:F0}MB", logFileSizeMb);
                }
                catch (OperationCanceledException) { break; }
                catch (Exception ex) { _logger.LogError(ex, "[BehavedrHealthCheck] Error"); }
