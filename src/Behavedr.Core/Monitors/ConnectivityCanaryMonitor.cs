// Connectivity Canary Monitor — detects network silencing of Behavedr (EDRSilencer, WFP blocking, DNS poisoning)
...
namespace Behavedr.Core
{
    ...
    /// 
    /// Detects:
    ///   - EDRSilencer WFP filters blocking Behavedr's outbound traffic
    ///   - DNS poisoning of proxy domain
    ///   - Firewall rules blocking Behavedr's traffic
    ...
    /// EDRSilencer tool can permanently blind Behavedr's cloud intelligence with zero alerts.
    /// </summary>
    public sealed class ConnectivityCanaryMonitor : BackgroundService
    {
        ...
        {
            _logger.LogInformation("[ConnectivityCanaryMonitor] Started — monitoring Behavedr cloud connectivity every 45s");
        ...
        /// <summary>
        /// Probes connectivity to Behavedr's threat intelligence endpoints.
        ...
        private async Task EmitSilencingAlertAsync(CancellationToken ct)
        {
            ...
            await _detectionEngine.EmitAsync(new DetectionEvent
            {
                ...
                Evidence = $"Behavedr cannot reach any threat intelligence endpoint. " +
                ...
                Reasoning = "Behavedr's outbound network connectivity to all threat intelligence APIs has been " +
                            "blocked. This matches the behavior of EDRSilencer (adds WFP filters to block EDR " +
                            "process outbound traffic), firewall-based EDR blinding, or DNS poisoning of Behavedr's " +
        ...
        private async Task EmitPersistentSilencingAlertAsync(CancellationToken ct)
        {
            ...
            await _detectionEngine.EmitAsync(new DetectionEvent
            {
                ...
                Evidence = $"Behavedr has been network-silenced for {silenceDuration.TotalMinutes:F0} minutes. " +
                           $"All threat intelligence endpoints remain unreachable. " +
                           $"Consecutive failures: {_consecutiveFailures}.",
                Reasoning = "Behavedr has been unable to reach any threat intelligence endpoint for an extended period. " +
                            "This strongly indicates an active EDR silencing tool (WFP filter, firewall rule, or DNS block) " +
                            "is persistently blocking Behavedr's outbound communications. All cloud-based detection " +
                            "capabilities (hash reputation, VirusTotal, ADS verdicts) are non-functional. " +
                            "Investigate WFP filters via 'netsh wfp show filters' and firewall rules targeting Behavedr.",
                Confidence = 0.95,
                Tier = DetectionTier.Tier1Behavioral,
                AuthorizedResponse = ResponseAction.LogOnly,
                ProcessName = "SYSTEM",
