...
namespace Behavedr.Core
{
    ...
    public sealed class CanaryFileMonitor : BackgroundService
    {
        ...
        private void PlantCanaryFiles()
        {
            ...
            foreach (var dir in dirs)
            {
                ...
                try
                {
                    var canary = Path.Combine(dir, ".~behavedr_canary.tmp");
                    if (!File.Exists(canary))
                    {
                        File.WriteAllText(canary, "BEHAVEDR_CANARY");
    ...
    public sealed class NullSessionGuard : BackgroundService
    {
        ...
        private const string FcmFirewallRuleName = "Behavedr-FCM-Push-Block";
            ...
            else if (anyChanged)
            {
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Reasoning = "The LimitBlankPasswordUse or RestrictAnonymous policy was found in a weakened state. " +
                                ...
                                "or a Group Policy override. Behavedr has re-applied the hardened settings.",
                    Confidence = 0.85,
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!ruleExists)
                    {
                        ...
                        newRule.Description = "Behavedr: Blocks Google FCM push notifications (port 5228) " +
    ...
    public sealed class BuiltinAdminGuard : BackgroundService
    {
        ...
        public BuiltinAdminGuard(DetectionEngine de, BehavedrConfig config, ILogger<BuiltinAdminGuard> l)
        {
            _detectionEngine = de;
            _config = config;
            _logger = l;
    ...
    //   - If no PIN is configured: Behavedr sets the screen lock timeout to "Never"
