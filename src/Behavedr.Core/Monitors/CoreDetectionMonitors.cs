                ...
                try
                {
                    ...
                    foreach (var (path, expectedHash) in _baselineHashes)
                    {
                        if (!File.Exists(path))
                        {
                            await _detectionEngine.EmitAsync(new DetectionEvent
                            {
                                RuleName = "Self-Protection: Behavedr Module Deleted",
                                Evidence = $"Module was deleted: {path}",
                                Reasoning = "A Behavedr runtime module was removed from disk, indicating active tampering.",
                        ...
                        if (currentHash != expectedHash)
                        {
                            await _detectionEngine.EmitAsync(new DetectionEvent
                            {
                                RuleName = "Self-Protection: Behavedr Module Tampered",
                                Evidence = $"Module hash mismatch: {path} (expected {expectedHash}, got {currentHash})",
                                Reasoning = "A Behavedr runtime module was modified on disk, indicating active tampering or DLL replacement.",
                                Confidence = 0.95, Tier = DetectionTier.Tier1Behavioral,
                                AuthorizedResponse = ResponseAction.KillProcessTree,
                                ProcessName = "SYSTEM", ProcessId = 0
                            });
        ...
        protected override async Task ExecuteAsync(CancellationToken ct)
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                try
                {
                    ...
                    // Check that the Behavedr service's own loaded modules are from expected paths
                        ...
                        try
                        {
                                ...
                                !modPath.Contains(@"\Microsoft\Windows Defender\", StringComparison.OrdinalIgnoreCase))
                            {
                                await _detectionEngine.EmitAsync(new DetectionEvent
                                {
                                    ...
                                    Evidence = $"Unexpected module loaded into Behavedr process: {modPath}",
                                    Reasoning = "A module from an untrusted path was loaded into the Behavedr service process, indicating possible DLL injection.",
                                    Confidence = 0.85, Tier = DetectionTier.Tier1Behavioral,
                                    AuthorizedResponse = ResponseAction.KillProcessTree,
                                    ProcessName = "Behavedr.Service", ProcessId = Environment.ProcessId
                                });
                            }
                        }
                        catch { }
