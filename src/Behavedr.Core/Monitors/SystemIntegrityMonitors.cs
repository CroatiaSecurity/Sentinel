...
namespace Behavedr.Core
{
    ...
    public sealed class TlsCertificateMonitor : BackgroundService
    {
        ...
        public TlsCertificateMonitor(
            DetectionEngine de, BehavedrConfig config, JsonlEventLogger logger,
        ...
        /// Unknown/suspicious certs that were present before Behavedr started get flagged as
            ...
            foreach (var (storeName, storeLocation, storeLabel) in storesToAudit)
            {
                ...
                try
                {
                    ...
                    foreach (var cert in store.Certificates)
                    {
                        ...
                        // This catches the "install cert before Behavedr starts" attack
                        if (analysis.Confidence >= 0.70)
                        {
                            ...
                            // These are almost certainly attacker MitM certs planted before Behavedr started
                    ...
                    if (anyChanged && !_initialEnforcement)
                    {
                        ...
                        await _detectionEngine.EmitAsync(new DetectionEvent
                        {
                            ...
                            Reasoning = "DNS-over-HTTPS in browsers and at the OS level bypasses the local hosts file entirely. " +
                                        ...
                                        "DoH is active. Behavedr disables DoH at every layer: Windows DNS client, Chrome, Edge, " +
            ...
            try
            {
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Reasoning = "The Windows hosts file controls local DNS resolution. Malware modifies it " +
                                ...
                                "Behavedr enforces the hardcoded trusted baseline at all times.",
                    Confidence = 0.95,
                    Tier = DetectionTier.Tier1Behavioral,
                    ...
                    try
                    {
                        ...
                        await _detectionEngine.EmitAsync(new DetectionEvent
                        {
                            ...
                            Reasoning = "Files like hosts.ics, lmhosts.sam, and others in drivers\\etc can be " +
                                        ...
                                        "client alongside hosts and is a known attack surface. Behavedr removes " +
