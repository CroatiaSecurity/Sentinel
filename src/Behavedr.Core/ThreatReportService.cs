    ...
    public class ThreatReportService
    {
        ...
        private static byte[] DeriveSigningKey(SecureCacheStore? cacheStore)
        {
            try
            {
                ...
                var entropyFile = System.IO.Path.Combine(programData, "Behavedr", "Secure", ".install_entropy");
                if (System.IO.File.Exists(entropyFile))
                {
                    ...
                    if (entropy.Length == 32)
                    {
                        ...
                        return hmac.ComputeHash(Encoding.UTF8.GetBytes("behavedr-threat-report-signing-v1"));
                    }
                }
            }
            catch { }
        ...
        private async Task SendReportAsync(string path, object payload)
        {
            try
            {
                ...
                request.Headers.Add("X-Behavedr-Timestamp", timestamp);
                request.Headers.Add("X-Behavedr-Signature", signature);
