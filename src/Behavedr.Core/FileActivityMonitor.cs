...
namespace Behavedr.Core
{
    public class FileActivityMonitor : IDisposable
    {
        ...
        private List<string> GetPathsToWatch(BehavedrConfig config)
        {
        ...
        private void OnFileEvent(object sender, FileSystemEventArgs e)
        {
                ...
                !pathLower.Contains(@"\drivers\etc\"))
            {
                ...
                {
                    // Only alert if the writer is NOT TrustedInstaller, Windows Update, Defender, or Behavedr itself
                    if (!IsTrustedSystemWriter(processInfo.pid, processInfo.name, e.FullPath) &&
                        !processInfo.name.Contains("Behavedr", StringComparison.OrdinalIgnoreCase) &&
        ...
        internal bool IsTrustedSystemWriter(int pid, string processName, string filePath)
        {
            ...
            var trustedNames = new[] { "trustedinstaller", "tiworker", "msiexec",
                ...
                // Behavedr itself
                "windowsbehavedr.service", "windowsbehavedr.agent",
                "windows behavedr" };
            ...
            // Own PID = Behavedr itself writing (e.g., quarantine lock files)
