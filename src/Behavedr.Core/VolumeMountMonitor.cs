...
namespace Behavedr.Core
{
    ...
    public sealed class VolumeMountMonitor : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken ct)
        {
            ...
            // Previous Behavedr versions or WMI enumeration side effects can cause Windows to
            ...
            // creates the SUBST drive before Behavedr starts to evade runtime-only detection.
        ...
        private static bool IsProtectedProcess(string name)
        {
            ...
            return lower is "system" or "smss" or "csrss" or "wininit" or "winlogon"
                ...
                or "windowsbehavedr" or "behavedrservice";
        }
        ...
        /// This ensures the attacker's drive doesn't come back after Behavedr removes it.
        /// </summary>
        ...
        /// Behavedr version's WMI enumeration side effect, or by Windows disk management bugs),
        ...
        private string? RunInUserSession(uint sessionId, string exePath, string arguments)
        {
            ...
            try
            {
                ...
                var tempFile = Path.Combine(Path.GetTempPath(), $"behavedr_subst_{sessionId}_{Guid.NewGuid():N}.tmp");
        ...
        private async Task EmitVolumeDetection(VolumeInfo vol, string classification)
        {
            await _detectionEngine.EmitAsync(new DetectionEvent
            {
                ...
                Reasoning = $"A new volume ({classification}) appeared after Behavedr startup. " +
