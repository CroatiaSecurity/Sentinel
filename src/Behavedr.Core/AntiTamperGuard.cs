...
namespace Behavedr.Core
{
    ...
    ///
    /// 1. Binary integrity — alerts if Behavedr's own executable is deleted/replaced while running
    /// 2. Anti-suspend detection — monitors execution timing; fires if a gap exceeds threshold
    ///    (indicates NtSuspendProcess was used to freeze Behavedr while attacker operates)
    /// 3. Service reinstall — if Behavedr's service registry key is deleted, re-registers via SCM
    ...
    public sealed class AntiTamperGuard : BackgroundService
    {
        ...
        private const string ServiceName = "Behavedr";
        // HARDENING: Lowered from 10s to 4s (2x tick interval). An attacker suspending
        // Behavedr for 9s had a full operating window undetected. Now any suspension
                    ...
                    if (elapsed > SuspendThresholdMs)
                    {
                        ...
                        await _detectionEngine.EmitAsync(new DetectionEvent
                        {
                            ...
                            Evidence = $"Execution gap of {gapSeconds:F1}s detected (expected ~{_timingTickMs / 1000.0:F1}s). " +
                                       $"Behavedr was likely suspended via NtSuspendProcess.",
                            Reasoning = "The Behavedr service experienced a timing gap far exceeding its " +
        ...
        private async Task CheckBinaryIntegrity()
        {
            ...
            if (!File.Exists(_ownExePath))
            {
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Evidence = $"Behavedr executable no longer exists at: {_ownExePath}",
                    Reasoning = "The Behavedr service binary has been deleted from disk while the service " +
                                "is still running. This is a direct tampering attempt — the attacker wants " +
                                "to ensure Behavedr cannot restart after a reboot or service crash.",
                    Confidence = 0.99,
                    Tier = DetectionTier.Tier1Behavioral,
        ...
        /// <summary>
        /// Checks if the Behavedr Windows service is still registered.
                ...
                if (_ownExePath != null)
                {
                    try
                    {
                        var psi = new ProcessStartInfo("sc.exe",
                            $"create \"{ServiceName}\" binPath=\"{_ownExePath}\" start=auto DisplayName=\"Behavedr\"")
                        { CreateNoWindow = true, UseShellExecute = false };
                        Process.Start(psi)?.WaitForExit(5000);
        ...
        /// any bandwidth throttling rules targeting Behavedr binaries.
        /// </summary>
            ...
            try
            {
                ...
                foreach (var subkeyName in baseKey.GetSubKeyNames())
                {
                    ...
                    if (!string.IsNullOrEmpty(appName) &&
                        (appName.Contains("Behavedr", StringComparison.OrdinalIgnoreCase) ||
                         appName.Contains("Behavedr", StringComparison.OrdinalIgnoreCase)))
                    {
                        // Found a policy targeting Behavedr! Delete it.
                        ...
                        await _detectionEngine.EmitAsync(new DetectionEvent
                        {
                            ...
                            Reasoning = "An attacker attempted to throttle or block Behavedr's network traffic " +
                                        ...
                                        "Behavedr has removed the registry key to restore full network bandwidth.",
                            Confidence = 0.99,
                            Tier = DetectionTier.Tier1Behavioral,
