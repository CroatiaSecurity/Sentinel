...
namespace Behavedr.Core
{
    public class AdvancedResponseEngine
    {
        ...
        public async Task HandleAsync(DetectionEvent detection)
        {
            ...
            if (detection.ProcessId > 0)
            {
                try
                {
                    ...
                    // (e.g., C:\Program Files\Behavedr2\evil.exe matching our dir).
                    ...
                    if (detectedImagePath != null)
                    {
                        ...
                        {
                            reason = "LogOnly (Self-exclusion: verified Behavedr install path)";
                            stopwatch.Stop();
        ...
        private void IsolateNetworkTarget(string ip, string ruleName)
        {
            ...
            var fwRule = $"Behavedr-Isolate-{safeName}";
            ...
            try
            {
                ...
                if (outRule != null)
                {
                    ...
                    outRule.Description = $"Behavedr: Block outbound to {ip} ({ruleName})";
                ...
                if (inRule != null)
                {
                    ...
                    inRule.Description = $"Behavedr: Block inbound from {ip} ({ruleName})";
