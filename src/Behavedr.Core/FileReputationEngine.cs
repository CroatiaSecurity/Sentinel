...
namespace Behavedr.Core
{
    ...
    public sealed class FileReputationEngine
    {
        ...
        // The Worker holds the VT API key server-side — Behavedr agents never see it.
        ...
        private async Task<ApiVerdict> QueryVirusTotalAsync(string hash, CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(_reportingConfig.ProxySharedSecret))
                {
                    ...
                    content.Headers.Add("X-Behavedr-Timestamp", timestamp);
                    content.Headers.Add("X-Behavedr-Auth", _reportingConfig.ProxySharedSecret);
                }
