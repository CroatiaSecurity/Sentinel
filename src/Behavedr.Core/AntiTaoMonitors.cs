...
namespace Behavedr.Core
{
    ...
    public sealed class HardwareSecurityGuard : BackgroundService
    {
        ...
        public HardwareSecurityGuard(DetectionEngine de, BehavedrConfig config, ILogger<HardwareSecurityGuard> l)
        {
    ...
    public sealed class DnsCrossValidator : BackgroundService
    {
        ...
        public DnsCrossValidator(DetectionEngine de, BehavedrConfig config, ILogger<DnsCrossValidator> l)
        {
            _detectionEngine = de; _config = config; _logger = l;
        }
    ...
    public sealed class UsbHidWhitelist : BackgroundService
    {
        ...
        // Hardcoded whitelist — in production, read from appsettings.json Behavedr.TrustedUsbDevices
        ...
        public UsbHidWhitelist(DetectionEngine de, BehavedrConfig config, ILogger<UsbHidWhitelist> l)
        {
            _detectionEngine = de; _config = config; _logger = l;
        }
    ...
    public sealed class TrafficVolumeBaseline : BackgroundService
    {
        ...
        public TrafficVolumeBaseline(DetectionEngine de, BehavedrConfig config, ILogger<TrafficVolumeBaseline> l)
        {
            _detectionEngine = de; _config = config; _logger = l;
        }
    ...
    public sealed class OutboundConnectionWhitelist : BackgroundService
    {
        ...
        // Hardcoded defaults — in production, read from Behavedr.AllowedOutboundSubnets
        ...
        private const string FirewallRuleName = "Behavedr-OutboundWhitelist-Block";

        public OutboundConnectionWhitelist(DetectionEngine de, BehavedrConfig config, ILogger<OutboundConnectionWhitelist> l)
        {
            _detectionEngine = de; _config = config; _logger = l;
        }
        ...
        protected override async Task ExecuteAsync(CancellationToken ct)
        {
            ...
            // Check enforcement mode from registry (Behavedr.OutboundWhitelistEnforced)
            try
            {
                using var key = Registry.LocalMachine.OpenSubKey(
                    @"SOFTWARE\Behavedr\Config");
        ...
        private void CreateFirewallBlockRule()
        {
            ...
            try
            {
                ...
                var allowPsi = new ProcessStartInfo("netsh.exe",
                    $"advfirewall firewall add rule name=\"Behavedr-OutboundWhitelist-Allow\" " +
