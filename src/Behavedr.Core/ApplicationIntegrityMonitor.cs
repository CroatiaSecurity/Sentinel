...
namespace Behavedr.Core
{
    ...
    public class ApplicationIntegrityConfig
    {
        ...
        /// Defaults to ProgramData\Behavedr\IntegrityBackups
        /// </summary>
        public string? BackupDirectory { get; set; }

        /// <summary>
        /// Directory where forensic incident reports are written.
        /// Defaults to ProgramData\Behavedr\IncidentReports
        /// </summary>
    ...
    public sealed class ApplicationIntegrityMonitor : BackgroundService
    {
        ...
        private readonly BehavedrConfig _behavedrConfig;
            ...
            ILogger<ApplicationIntegrityMonitor> logger)
        {
            ...
            _config = config;
            _behavedrConfig = behavedrConfig;
            _logger = logger;

            var programData = Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData);
            _backupDir = config.BackupDirectory
                ?? Path.Combine(programData, "Behavedr", "IntegrityBackups");
            _reportDir = config.ReportDirectory
                ?? Path.Combine(programData, "Behavedr", "IncidentReports");
        ...
        private async Task CheckSingleApplicationAsync(string executablePath)
        {
            ...
            try
            {
                ...
                if (!string.Equals(currentHash, baseline.Sha256Hash, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (_behavedrConfig.ActiveResponse)
                    {
            ...
            string impostorHash, string? impostorPublisher, string? impostorProduct, int offenderPid)
        {
            ...
            report.AppendLine("╔══════════════════════════════════════════════════════════════════╗");
            report.AppendLine("║     WINDOWS BEHAVEDR — FORENSIC INCIDENT REPORT                ║");
            ...
            AppBaseline baseline, DateTime detectionTime)
        {
            ...
            report.AppendLine($"  {detectionTime:yyyy-MM-dd HH:mm:ss} UTC — Unauthorized replacement detected by Behavedr");
            ...
            report.AppendLine("  2. The quarantined impostor binary is DPAPI-encrypted in the Behavedr");
            ...
            report.AppendLine();
            report.AppendLine("═══════════════════════════════════════════════════════════════════");
            report.AppendLine("         Generated by Behavedr Application Integrity Monitor");
            report.AppendLine("═══════════════════════════════════════════════════════════════════");
