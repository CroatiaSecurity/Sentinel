...
namespace Behavedr.Agent
{
    public class TrayIconService : IHostedService, IDisposable
    {
        ...
        public TrayIconService(BehavedrConfig config)
        {
            _config = config;
        ...
        private void RunUiThread()
        {
            ...
            // must stop the Behavedr service via an elevated command prompt.
            _contextMenu.Items.Add("Exit Agent", null, OnExit);

            System.Drawing.Icon? appIcon = null;
            try
            {
                // Primary: load the deployed Behavedr.ico from the application directory
                var icoPath = Path.Combine(AppContext.BaseDirectory, "Behavedr.ico");
            ...
            _notifyIcon = new NotifyIcon
            {
                ...
                Text = $"Behavedr v{_version} — Protection Active",
                Visible = true
            };
        ...
        private void OnOpenConsole(object? sender, EventArgs e)
        {
            try
            {
                ...
                var logFile = Path.Combine(programData, "Behavedr", "events.jsonl");
        ...
        private void OnOpenQuarantine(object? sender, EventArgs e)
        {
            ...
            {
                var programData = Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData);
                var qDir = Path.Combine(programData, "Behavedr", "Quarantine");
