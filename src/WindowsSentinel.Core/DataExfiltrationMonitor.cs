...
/// <summary>
/// Data Exfiltration Prevention Monitor (v1.8.0) â€” Correlation-based DLP.
/// 
...
/// 
/// How it works:
///   1. Monitors DNS resolutions for known exfil service domains (via DnsQueryMonitor â€” Tier2)
///   2. Monitors sensitive file access patterns (credential stores, SSH keys â€” Tier2)
///   3. Monitors removable media reads (USB drives â€” Tier2)
...
public sealed class DataExfiltrationMonitor : BackgroundService
{
    ...
    private readonly ConcurrentDictionary<string, DateTime> _alertedExfil = new();
    ...
    // Default protected paths â€” covers common high-value data locations.
    ...
    private static readonly string[] DefaultProtectedPaths = new[]
    {
        ...
        // All non-C: fixed drives (D:, E:, F:, etc.) â€” likely data drives
        // These are resolved dynamically at startup
    };

    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // ALLOWLISTS â€” processes that legitimately do these things
    // 
    ...
    // Trusted install locations â€” binaries here get reduced suspicion
    ...
    private static readonly HashSet<string> NetworkAllowlist = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "ctfmon", "fontdrvhost",
        // Games (common launchers â€” individual games are covered by trusted path check)
    ...
    // (broader than credential allowlist â€” includes file managers, backup tools, etc.)
    ...
    // Disk image extensions â€” high-value exfil targets
    ...
    // Sensitive path fragments â€” credential stores and key material
    ...
    {
        _logger.LogInformation("[DataExfiltrationMonitor] Starting â€” correlation-based exfil prevention active");
    ...
    private async Task ScanEstablishedConnectionsAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < numEntries; i++)
            {
                ...
                // Skip allowlisted processes running from trusted paths â€” no signal, no noise
                ...
                if (duration >= TimeSpan.FromSeconds(60) && !tracker.Alerted)
                {
                    ...
                    _alertedExfil.TryAdd(dedupeKey, DateTime.UtcNow);

                    // Tier2 â€” this alone does NOT kill. Needs correlation.
                    await _detectionEngine.EmitAsync(new DetectionEvent
                    {
                        ...
                        Reasoning = "A process not in the network allowlist has maintained a sustained outbound " +
                                    "connection to an external IP. This is a corroborating signal â€” combined with " +
    ...
    private void OnRemovableFileEvent(string filePath, string drivePath, CancellationToken ct)
    {
        ...
        // Tier2 â€” log only. Correlation engine will combine with network signals for kill.
    ...
    private void OnSensitiveFileEvent(string filePath, CancellationToken ct)
    {
        ...
        // Tier2 â€” log only. Correlation engine combines with network for kill.
        _ = _detectionEngine.EmitAsync(new DetectionEvent
        {
            RuleName = "Sensitive File Access: Credential Store",
            Evidence = $"Credential/key file accessed: {filePath}",
            Reasoning = $"A sensitive {category} file was accessed. This is a corroborating signal â€” " +
    ...
    private static bool IsProcessTrusted(int pid, string? processName, HashSet<string> allowlist)
    {
        ...
        // Name matches â€” now verify it's running from a trusted location
        try
        {
            ...
            {
                // Can't get path (sandboxed/protected process) â€” trust known Microsoft components
            ...
            // NOT in a trusted path â€” don't trust it even if name matches
            return false;
        }
        catch
        {
            // Can't verify path â€” don't trust
            return false;
        }
    }
