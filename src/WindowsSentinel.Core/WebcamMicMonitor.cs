...
/// <summary>
/// Webcam &amp; Microphone Exfiltration Monitor â€” Detects unauthorized background
...
///   2. Processes WITH a visible window are given a pass â€” the user can see them
...
///   5. Standalone detection is Tier2 (log only) â€” only the composite with network
///      activity triggers a kill (via correlation engine)
///
/// This means:
///   - User on Google Meet in Chrome â†’ Chrome is allowlisted â†’ no alert
///   - User streaming on OBS â†’ OBS is allowlisted â†’ no alert
///   - User in Zoom call â†’ Zoom is allowlisted â†’ no alert
///   - Unknown background process loading camera DLLs + sending data â†’ ALERT + KILL
///
/// Feeds into BehavioralCorrelationEngine composites:
///   - WebcamMic + Network â†’ "Camera/Mic Exfiltration" (spyware streaming to C2)
///   - WebcamMic + ScreenCapture â†’ "Full Surveillance Suite" (updated to include camera)
///
...
public sealed class WebcamMicMonitor : BackgroundService
{
    ...
    // â”€â”€ DLLs that indicate webcam/camera access â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    {
        "mfcore.dll",           // Media Foundation core (Win10+) â€” less common than mfplat
    ...
    /// (as opposed to playback â€” we only care about capture/recording).
    ...
    {
        "audioclient.dll",      // Audio client (capture streams â€” more specific than audioses)
        "mfreadwrite.dll",      // Media Foundation read/write (recording)
        "winmm.dll",            // Windows Multimedia (waveIn API â€” legacy recording)
        "portaudio.dll",        // PortAudio (cross-platform audio capture)
        "naudio.dll",           // NAudio (.NET audio library)
    };

    /// <summary>
    /// Stronger camera indicators â€” these DLLs are specifically for camera
    /// frame capture, not general media playback.
    /// </summary>
    private static readonly string[] StrongCameraIndicators =
    {
        "mfsensorgroup.dll",    // Camera sensor group â€” only loaded for camera access
        "frameserver.dll",      // Frame Server â€” camera frame delivery
    ...
    // â”€â”€ Allowlisted processes (legitimate camera/mic users) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ P/Invoke for window visibility check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private async Task ScanAsync(CancellationToken ct)
    {
        ...
        try
        {
            foreach (var proc in procs)
            {
                ...
                // Skip processes with a visible main window â€” user can see them
                ...
                // Confirmation threshold â€” must be seen across multiple scans
                var count = _hitCount.AddOrUpdate(p.Id, 1, (_, c) => c + 1);
                if (count < ConfirmationThreshold) continue;

                // This is a background process with camera/mic access â€” emit detection
                _alertedPids[p.Id] = DateTime.UtcNow;
                ...
                _logger.LogWarning(
                    ...
                    "with no visible window â€” possible spyware/RAT",
                ...
                _fusionEngine?.IngestFileActivity(p.Id, p.ProcessName,
                    "webcam_mic_capture", FileActivityKind.Read, DateTime.UtcNow);
            }
        }
        finally
        {
