...
/// <summary>
/// DNS Query Monitor â€” Subscribes to the Microsoft-Windows-DNS-Client ETW provider
/// to observe every DNS resolution on the system.
///
/// This catches:
///   - C2 over DNS (iodine, dnscat2, dns2tcp)
///   - DGA domains (high-entropy random names)
///   - Process â†’ domain correlation ("process X resolved suspicious.domain then connected")
...
public sealed class DnsQueryMonitor : IMonitor
{
    ...
    // Processes excluded from DNS tunneling detection â€” these legitimately make
    ...
    // Domain suffixes excluded from DNS tunneling / DGA detection â€” known-good high-query domains
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(
                "[{Monitor}] Access denied â€” elevation required. DNS monitoring disabled.", Name);
            _session?.Dispose();
            _session = null;
        }
        catch (Exception ex)
    ...
    private void HandleDnsEvent(TraceEvent data, CancellationToken ct)
    {
            ...
            _blocklistEngine.IsBlocked(queryName, out var blockCategory, out var blockReason))
        {
            ...
            {
                BlocklistCategory.C2 => 0.92,       // C2 infrastructure â€” high confidence
                BlocklistCategory.Malware => 0.90,  // Payload delivery â€” high confidence
                BlocklistCategory.Phishing => 0.85, // Credential theft â€” slightly lower (legit domains get compromised)
                _ => 0.85
            };
        ...
        if ((DateTime.UtcNow - _lastCleanup).TotalMinutes > 2)
        {
            CleanupStats();
        ...
        stats.TotalQueries++;
        stats.LastQuery = DateTime.UtcNow;
            ...
            if (!IsBrowserOrAllowlisted(processName))
            {
                ...
                _ = _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Reasoning = "Non-browser processes resolving file-sharing, paste, or upload service domains " +
                               "indicates potential data exfiltration staging. This is a corroborating signal â€” " +
                               "combined with outbound network activity, it confirms active exfil.",
                    Confidence = Math.Min(0.70 + (stats.ExfilDomainHits * 0.05), 0.85),
                    Tier = DetectionTier.Tier2Indicator, // Corroborating â€” needs network correlation to kill
                    ProcessName = processName ?? processPid.ToString(),
                    ProcessId = processPid,
                    Timestamp = DateTime.UtcNow,
                    Metadata = new Dictionary<string, string>
            ...
            if (entropy >= DgaEntropyThreshold)
            {
                ...
                if (stats.DgaHits >= 3)
                {
                    _ = _detectionEngine.EmitAsync(new DetectionEvent
                    {
                        ...
                        Tier = DetectionTier.Tier2Indicator, // Corroborating signal â€” needs correlation to kill
        ...
        {
            var elapsed = (DateTime.UtcNow - stats.FirstSeen).TotalMinutes;
            if (elapsed > 0)
            {
    ...
    private void BlockDomainViaFirewall(string domain, int triggerPid)
    {
        try
        {
            ...
            catch
            {
                ...
                // Detection was already emitted â€” this is just the blocking response
                return;
            }
            ...
            foreach (var ip in addresses)
            {
                ...
                try
                {
                    ...
                    _logger.LogCritical(
                        "[DnsBlocklist] FIREWALL BLOCK: {Domain} â†’ {IP} (rule: {Rule})",
...
internal sealed class DnsQueryStats
{
    ...
    public DateTime LastQuery { get; set; } = DateTime.UtcNow;
