...
/// <summary>
/// Screen Capture &amp; Overlay Security Monitor — Detects unauthorized screen capture,
...
///      owned by non-allowlisted processes — used for credential phishing overlays
///   3. Background processes loading screen-capture DLL combinations (gdi32 + d3d11 + dxgi)
///      without a visible window — indicates silent screen grabbing
...
///   - ScreenCapture + Network → "Screen Exfiltration" (spyware)
///   - ScreenCapture + Clipboard → "Data Harvesting" (infostealer)
///   - Overlay + Injection → "Credential Phishing via Overlay"
/// </summary>
public sealed class ScreenCaptureMonitor : BackgroundService
{
    ...
    private readonly ConcurrentDictionary<int, DateTimeOffset> _alertedCapturePids = new();
    private readonly ConcurrentDictionary<int, DateTimeOffset> _alertedOverlayPids = new();
    ...
    // ── DLL combinations that indicate screen capture capability ──────────────

    /// <summary>
    /// Primary screen capture indicators — processes loading these are likely capturing.
    /// </summary>
    ...
    /// <summary>
    /// Secondary indicators — combined with primary = stronger signal.
    /// </summary>
    ...
    // ── Allowlisted processes (legitimate screen capture) ─────────────────────
    ...
    // ── Allowlisted overlay processes (legitimate topmost transparent windows) ─
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var lastOverlayScan = DateTimeOffset.MinValue;
    ...
    private async Task ScanForScreenCaptureProcessesAsync(CancellationToken ct)
    {
        ...
        try
        {
            foreach (var proc in procs)
            {
                ...
                // (just having dxgi.dll alone is too common — every GPU app loads it)
                ...
                _alertedCapturePids[p.Id] = DateTimeOffset.UtcNow;
                ...
                _logger.LogWarning(
                    ...
                    "DLL combination loaded without a visible window — possible screen-grabbing malware",
                    p.ProcessName, p.Id);
                ...
                _fusionEngine?.IngestFileActivity(p.Id, p.ProcessName,
                    "screen_capture", FileActivityKind.Read, DateTimeOffset.UtcNow);
            }
        }
        finally
        {
    ...
    private void ScanForSuspiciousOverlays(CancellationToken ct)
    {
        ...
        EnumWindows((hWnd, _) =>
        {
            ...
            // Get window size — ignore tiny windows (tooltips, notification icons)
            if (!GetWindowRect(hWnd, out RECT rect)) return true;
            if (rect.Area < 50000) return true; // Less than ~224x224 pixels — too small to be an overlay attack
        ...
        foreach (var (hWnd, pid, processName, rect) in suspiciousWindows)
        {
            ...
            // Increment hit count — only alert after confirmation threshold
            var count = _overlayHitCount.AddOrUpdate(pid, 1, (_, c) => c + 1);
            if (count < OverlayConfirmationThreshold) continue;

            _alertedOverlayPids[pid] = DateTimeOffset.UtcNow;
            ...
            _logger.LogWarning(
                ...
                "({W}x{H}) — possible credential phishing or banking trojan overlay",
            ...
            _fusionEngine?.IngestFileActivity(pid, processName,
                "overlay_window", FileActivityKind.Write, DateTimeOffset.UtcNow);
        }
