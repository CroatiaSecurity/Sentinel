...
/// <summary>
/// Firefox Credential Guard Monitor — Detects unauthorized access to Firefox/Gecko
...
///   - cookies.sqlite: Session cookies (unencrypted SQLite — high-value target)
...
///   Stealer → copies key4.db + logins.json → uses NSS library to decrypt → extracts passwords
///   OR: Stealer → copies cookies.sqlite directly → cookies are NOT encrypted (unlike Chrome)
///
...
public sealed class FirefoxCredentialGuardMonitor : BackgroundService
{
    ...
    /// <summary>
    /// Critical files — access by non-Firefox processes is ALWAYS suspicious.
    /// </summary>
    private static readonly string[] CriticalFiles =
    {
        ...
        "cookies.sqlite",       // Session cookies (UNENCRYPTED — high value)
    ...
    private static readonly (string BrowserName, string RelativePath)[] FirefoxPaths =
    {
        ...
        ("Thunderbird",  @"Thunderbird\Profiles"),  // Email client — stores email account credentials
    };
                ...
                !fileName.Equals(critical, StringComparison.OrdinalIgnoreCase))
            {
                _ = Task.Run(async () =>
                {
                    await EmitDetectionAsync(
                        ...
                        "cookies are stored in PLAINTEXT SQLite — no decryption needed. This makes Firefox " +
    ...
    private async Task CheckFileAccessAsync(string filePath, string fileName, CancellationToken ct)
    {
        ...
        foreach (var process in processes)
        {
            try
            {
                ...
                // Check process age — young processes accessing Firefox data are suspicious
                TimeSpan processAge;
                ...
                if (processAge < TimeSpan.FromMinutes(2))
                {
                    ...
                    await EmitDetectionAsync(
                        ...
                        "Firefox's cookies.sqlite requires NO decryption — a simple file copy gives the attacker " +
