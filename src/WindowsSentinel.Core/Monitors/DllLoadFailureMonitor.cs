...
/// <summary>
/// DLL Load Failure Monitor — Monitors Windows Event Log for DLL load failures
...
///
/// Also monitors:
///   - Event ID 11 (SideBySide) — manifest/activation context errors
///   - Event ID 59 (Application Error) — DLL initialization failures
///
/// MITRE ATT&CK:
///   T1574 — Hijack Execution Flow
///   T1055 — Process Injection (failed attempts)
/// </summary>
public sealed class DllLoadFailureMonitor : BackgroundService
{
    ...
    private async Task PollEventLogAsync(CancellationToken ct)
    {
        ...
        var queryTime = _lastPollTime.UtcDateTime;
        _lastPollTime = now;

        // Query System log for Event ID 7 (Service Control Manager — DLL load failures)
            ...
            while ((record = reader.ReadEvent()) != null)
            {
                ...
                using (record)
                {
                    ...
                    await _detectionEngine.EmitAsync(new DetectionEvent
                    {
                        ...
                        ProcessId = 0,
                        Timestamp = new DateTimeOffset(timeCreated, TimeSpan.Zero),
    ...
    /// which indicate manifest/activation context failures — often caused by
