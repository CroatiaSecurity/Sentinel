...
/// <summary>
/// Microsoft Account Guard Monitor — Detects unauthorized access to Microsoft account
...
///   1. WAM (Web Account Manager) tokens — stored in TokenBroker cache
///   2. Azure AD / Entra ID tokens — stored in WAM + registry
///   3. Office 365 tokens — stored in Credential Manager + registry
///   4. OneDrive tokens — stored in registry + SQLite
///   5. Windows Hello credentials — stored in NGC container
///   6. Microsoft Edge profile tokens — stored in Edge User Data
///
...
public sealed class MicrosoftAccountGuardMonitor : BackgroundService
{
    ...
    private readonly ConcurrentDictionary<string, DateTimeOffset> _alertedAccess = new();
    ...
    /// <summary>
    /// TokenBroker cache — contains WAM tokens including Primary Refresh Tokens (PRT).
    ...
    /// <summary>
    /// Windows NGC (Next Generation Credentials) — Windows Hello data.
    /// </summary>
    ...
    private static readonly string[] MsftTokenTheftPatterns =
    {
        ...
        "get-prttoken",
        "browsercore.exe",              // BrowserCore — used for PRT extraction
            ...
            try
            {
                ...
                if (matchedPattern != null)
                {
                    ...
                    await EmitDetectionAsync(
                        ...
                        "Azure resources, and more — without needing the password or triggering MFA.",
                        0.93,
                        DetectionTier.Tier1Behavioral,
    ...
    /// PRT is the crown jewel — it provides SSO to ALL Microsoft services.
    /// </summary>
    private async Task ScanForPrtExtractionAsync(CancellationToken ct)
    {
        ...
        foreach (var process in processes)
        {
            ...
            try
            {
                ...
                // BrowserCore.exe is used for PRT extraction — should only be spawned by browsers
