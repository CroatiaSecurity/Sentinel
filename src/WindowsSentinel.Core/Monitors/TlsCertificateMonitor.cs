...
/// <summary>
/// TLS Certificate Transparency Monitor (v3.6.0) — Detects MITM via certificate anomalies.
///
...
public sealed class TlsCertificateMonitor : BackgroundService
{
    ...
    {
        _logger.LogInformation("[TlsCertificateMonitor] Starting — TLS certificate integrity monitoring active");
    ...
    private async Task CheckCertificatesAsync(CancellationToken ct)
    {
        foreach (var target in CheckTargets)
        {
            ...
            {
                // Connection failed — network issue, not necessarily an attack
            }
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
    ...
    private async Task ValidateCertificateAsync(CertCheckTarget target, CertInfo cert, CancellationToken ct)
    {
                ...
                else
                {
                    await _detectionEngine.EmitAsync(new DetectionEvent
                    {
                        ...
                        Reasoning = "A major domain's TLS certificate was issued by an unexpected Certificate " +
                                    "Authority. This indicates TLS interception — someone is performing a " +
        ...
        // ═══════════════════════════════════════════════════════════════════
        // CHECK 3: Certificate pinning — fingerprint changed from baseline
        // ═══════════════════════════════════════════════════════════════════
        ...
        {
            // First time seeing this domain — capture baseline
    ...
    private async Task<CertInfo?> GetCertificateInfoAsync(string url, CancellationToken ct)
    {
        ...
        client.DefaultRequestHeaders.UserAgent.ParseAdd("WindowsSentinel/3.8.0");

        try
        {
            // Just need to establish TLS — don't need to read the response
