...
/// <summary>
/// Chrome Session Guard Monitor — Detects active session hijacking attacks against
...
///   Stealer → reads Local State → extracts encrypted_key → DPAPI decrypt → reads Cookies DB → decrypts cookies → exfiltrates
///
...
///   T1055 — Process Injection (into browser)
/// </summary>
public sealed class ChromeSessionGuardMonitor : BackgroundService
{
    ...
    /// Newer Chrome versions use elevation_service to encrypt cookies — attackers abuse it to decrypt.
    /// </summary>
    ...
    private bool ShouldAlert(string key)
    {
        ...
        {
            if (DateTimeOffset.UtcNow - last < TimeSpan.FromMinutes(5))
                return false;
        }
