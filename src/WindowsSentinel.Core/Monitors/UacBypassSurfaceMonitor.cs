...
/// <summary>
/// UAC Bypass Surface Monitor — Proactively scans for DLL hijacking vectors that
...
///      are writable or missing — attacker can plant a DLL to get auto-elevated execution.
///   2. System32 binaries with autoElevate manifest that lack SetDllDirectory hardening —
                    ...
                    if (isVulnerable)
                    {
                        ...
                        await _detectionEngine.EmitAsync(new DetectionEvent
                        {
                            ...
                            Evidence = $"Binary '{fileName}' has autoElevate manifest and lacks " +
                                      "SetDllDirectory/SetDefaultDllDirectories hardening — " +
                                      "vulnerable to copy-to-temp DLL sideload UAC bypass.",
        ...
        foreach (var dir in pathDirs)
        {
            ...
            try
            {
                var recentDlls = Directory.GetFiles(dir, "*.dll", SearchOption.TopDirectoryOnly)
                    ...
                    .Where(f => (DateTimeOffset.UtcNow - f.LastWriteTimeUtc).TotalHours < 24)
                    .Take(20);
