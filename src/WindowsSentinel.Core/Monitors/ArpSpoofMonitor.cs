...
/// <summary>
/// ARP Spoof Detection Monitor (v3.6.0) — Detects ARP table manipulation.
///
...
///
/// Detection strategy:
///   1. On startup, capture the gateway IP → MAC binding as baseline.
...
///
/// This monitor runs in the SYSTEM service context — it has full access to
...
public sealed class ArpSpoofMonitor : BackgroundService
{
    ...
    // Baseline: gateway IP → MAC mapping captured at startup
    private readonly ConcurrentDictionary<string, string> _gatewayBaseline = new();

    // Track all IP → MAC mappings for duplicate detection
    ...
    {
        _logger.LogInformation("[ArpSpoofMonitor] Starting — ARP table integrity monitoring active");
            ...
            foreach (var gwIp in gateways)
            {
                if (arpEntries.TryGetValue(gwIp, out var mac))
                {
                    ...
                    _logger.LogInformation(
                        "[ArpSpoofMonitor] Baseline captured — Gateway {Ip} → MAC {Mac}",
                        gwIp, mac);
                }
            }
    ...
    private async Task ScanArpTableAsync(CancellationToken ct)
    {
        ...
        foreach (var gwIp in gateways)
        {
            ...
            if (!_gatewayBaseline.ContainsKey(gwIp))
            {
                ...
                _logger.LogInformation("[ArpSpoofMonitor] Late baseline — Gateway {Ip} → MAC {Mac}", gwIp, currentMac);
                continue;
            }
            ...
            if (!string.Equals(currentMac, baselineMac, StringComparison.OrdinalIgnoreCase))
            {
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Evidence = $"Gateway {gwIp} MAC changed from {baselineMac} to {currentMac}. " +
                               "This indicates an ARP spoofing attack — an attacker on the local network " +
        ...
        // Suspicious on physical hardware — could indicate VM-based MITM.
        // ═══════════════════════════════════════════════════════════════════
        foreach (var gwIp in gateways)
        {
