...
/// <summary>
/// Device Installation Monitor (v4.2.0) — Detects new device installations including:
///
...
///   6. Hidden/phantom devices (devices present but not connected — attacker persistence)
...
///
/// Threat model:
///   - Attacker with admin access installs virtual keyboard → keystroke injection
///   - Attacker installs TAP adapter → traffic interception/MITM
///   - Attacker mounts iSCSI/virtual disk → data staging or payload delivery
///   - Attacker loads vulnerable driver (BYOVD) → kernel-level access
///   - Attacker installs hidden device for persistence (survives reboot, invisible in Device Manager)
///
/// MITRE ATT&amp;CK:
///   T1200 — Hardware Additions
///   T1056.001 — Input Capture: Keylogging (via virtual HID)
///   T1557 — Adversary-in-the-Middle (via rogue network adapter)
///   T1543.003 — Create or Modify System Process: Windows Service (driver load)
///   T1068 — Exploitation for Privilege Escalation (BYOVD)
///   T1564.001 — Hide Artifacts: Hidden Files and Directories (ghost devices)
/// </summary>
public sealed class DeviceInstallMonitor : BackgroundService
{
    ...
    // Deduplication
    private readonly ConcurrentDictionary<string, DateTimeOffset> _alertedDevices = new();
    private readonly ConcurrentDictionary<string, DateTimeOffset> _alertedDrivers = new();
    ...
    {
        _logger.LogInformation("[DeviceInstall] Starting — baselining installed devices and drivers");
    ...
    private void HandleNewDevice(string deviceId, string name, string classGuid, string manufacturer, CancellationToken ct)
    {
        ...
        _alertedDevices[deviceId] = DateTimeOffset.UtcNow;
        ...
        _logger.LogWarning(
            "[DeviceInstall] NEW DEVICE: {Category} — '{Name}' (Class: {Class}, ID: {Id})",
    ...
    private void HandleNewDriver(string name, string displayName, string pathName, CancellationToken ct)
    {
        ...
        _alertedDrivers[name] = DateTimeOffset.UtcNow;
        ...
        {
            // System driver path — likely Windows Update. Log but don't alert high.
            _logger.LogInformation("[DeviceInstall] New system driver loaded: {Name} ({Path})", name, pathName);
            return;
        }

        // Non-system driver loaded at runtime — suspicious
        ...
        _logger.LogWarning(
            "[DeviceInstall] NEW KERNEL DRIVER: '{Display}' ({Name}) — Path: {Path}",
        ...
        _ = _detectionEngine.EmitAsync(new DetectionEvent
        {
            ...
            Reasoning = "Kernel drivers have unrestricted access to the system. Loading a new driver " +
                "at runtime (after boot) is unusual — legitimate drivers are typically loaded during " +
        ...
        try
        {
            ...
            foreach (ManagementObject device in searcher.Get())
            {
                    ...
                    category == DeviceCategory.NetworkAdapter)
                {
                    ...
                    _alertedDevices[dedupeKey] = DateTimeOffset.UtcNow;

                    _logger.LogWarning(
                        "[DeviceInstall] HIDDEN DEVICE: {Category} — '{Name}' (Error: {Error}, Status: {Status})",
                    ...
                    _ = _detectionEngine.EmitAsync(new DetectionEvent
                    {
                        ...
                        Evidence = $"Hidden {category} device found: '{name}'. " +
                                  ...
                                  $"This device is registered but not currently active — it may be " +
    ...
    //
    // This is equivalent to "Show hidden devices" in Device Manager → right-click → Uninstall
    // ═══════════════════════════════════════════════════════════════════════════
