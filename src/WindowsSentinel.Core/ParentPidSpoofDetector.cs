...
/// <summary>
/// Parent PID Spoofing Detector â€” Detects processes that fake their parent PID
...
///
/// Near-zero false positives â€” legitimate Windows never disagrees on parent PID.
...
public sealed class ParentPidSpoofDetector
{
    ...
    // Processes that legitimately act as intermediaries (declared parent â‰  real parent)
    ...
    public async Task CheckForSpoofingAsync(CancellationToken ct)
    {
        var toCheck = _etwParents.Values
            .Where(r => (DateTime.UtcNow - r.Timestamp).TotalSeconds is > 1 and < 30)
            .ToList();
        ...
        foreach (var record in toCheck)
        {
            ...
            var declaredParentName = ancestors.Count > 1 ? ancestors[1].name : string.Empty; // ancestors[0] is the process itself
            ...
            if (LegitimateIntermediaries.Contains(etwParentName) ||
                LegitimateIntermediaries.Contains(declaredParentName ?? string.Empty))
                continue;
            ...
            // SPOOFING DETECTED
            _logger.LogCritical(
                "PPID SPOOF DETECTED: '{Child}' (PID {ChildPid}) â€” " +
            ...
            await _detectionEngine.EmitAsync(new DetectionEvent
            {
                ...
                Confidence = 0.95,
                Tier = DetectionTier.Tier2Indicator, // Corroborating signal â€” feeds correlation engine, not a standalone kill
