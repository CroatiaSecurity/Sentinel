...
public sealed class ModuleValidationMonitor : BackgroundService
{
            ...
            foreach (var proc in procs)
            {
                ...
                foreach (ProcessModule m in modules)
                {
                    ...
                    await _engine.EmitAsync(new DetectionEvent
                    {
                        ...
                        Evidence = $"{p.ProcessName} (PID {p.Id}) loaded {fileName} â€” score {score}",
    ...
    private ModuleInfo? GetOrComputeModuleInfo(string path)
    {
        if (_moduleCache.TryGetValue(path, out var existing) &&
            DateTime.UtcNow - existing.VerifiedAt < ModuleCacheTtl)
        {
            return existing;
        }
        ...
        var info = new ModuleInfo { Path = path, VerifiedAt = DateTime.UtcNow };
    ...
    {
        var cutoff = DateTime.UtcNow - ModuleCacheTtl;
