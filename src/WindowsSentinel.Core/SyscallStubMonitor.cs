...
/// <summary>
/// Syscall Stub Integrity Monitor â€” Detects ntdll.dll unhooking in the Sentinel process.
///
...
/// in our process â€” either way, it's an integrity violation.
///
/// Monitored functions:
...
///
/// This runs ONLY on our own process â€” no cross-process access needed.
/// </summary>
public sealed class SyscallStubMonitor : BackgroundService
{
    ...
    // Functions to monitor: module â†’ export name
    ...
    private async Task CheckIntegrityAsync(CancellationToken ct)
    {
        foreach (var (module, export) in MonitoredFunctions)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogCritical(
                        "SYSCALL STUB TAMPERED: {Module}!{Export} â€” bytes changed from baseline. " +
