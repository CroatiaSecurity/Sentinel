...
/// <summary>
/// Windows Firewall Integrity Monitor (v3.6.0) â€” Detects firewall tampering.
///
...
///
/// Detection strategy:
///   1. Monitor firewall profile states (Domain/Private/Public) â€” alert if disabled.
...
public sealed class FirewallIntegrityMonitor : BackgroundService
{
    ...
    {
        _logger.LogInformation("[FirewallIntegrityMonitor] Starting â€” firewall integrity monitoring active");
            ...
            if (!_alertedEvents.ContainsKey(dedupeKey))
            {
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Evidence = "The Windows Firewall service (mpssvc) is NOT running. " +
                               "All firewall rules are inactive â€” the system has no network filtering.",
