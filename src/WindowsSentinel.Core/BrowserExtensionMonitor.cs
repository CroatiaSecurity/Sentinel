...
/// <summary>
/// Browser Extension Monitor â€” Detects silent installation of malicious Chrome extensions.
///
/// Protects against:
...
public sealed class BrowserExtensionMonitor : BackgroundService
{
    ...
    private readonly ConcurrentDictionary<string, DateTime> _alertedExtensions = new();
    ...
    private void OnExtensionFileCreated(object sender, FileSystemEventArgs e)
    {
        ...
        // New manifest.json created â€” possible new extension installation
        _ = Task.Run(async () =>
        {
    ...
    private async Task AnalyzeNewExtensionAsync(string manifestPath, string manifestContent, CancellationToken ct)
    {
        try
        {
            ...
            // Check if browser is running (if not, this is more suspicious â€” silent install)
            bool browserRunning = IsBrowserRunning();
            if (!browserRunning)
            {
                confidence = Math.Min(confidence + 0.10, 0.98);
                suspiciousReason = (suspiciousReason ?? "") + " [Installed while browser was NOT running â€” silent install]";
            }

            var extensionId = GetExtensionIdFromPath(manifestPath);
            _alertedExtensions[extensionId] = DateTime.UtcNow;
        ...
        {
            // Malformed manifest â€” could be obfuscated malware
        ...
        foreach (var regPath in ExtensionRegistryPaths)
        {
            try
            {
                ...
                foreach (var valueName in key.GetValueNames())
                {
                    ...
                    _alertedExtensions[alertKey] = DateTime.UtcNow;
