...
namespace WindowsSentinel.Core
{
    ...
    public class ApplicationIntegrityConfig
    {
        ...
        /// Defaults to ProgramData\WindowsSentinel\IntegrityBackups
        /// </summary>
        public string? BackupDirectory { get; set; }

        /// <summary>
        /// Directory where forensic incident reports are written.
        /// Defaults to ProgramData\WindowsSentinel\IncidentReports
        /// </summary>
    ...
    public sealed class ApplicationIntegrityMonitor : BackgroundService
    {
            ...
            ILogger<ApplicationIntegrityMonitor> logger)
        {
            ...
            _backupDir = config.BackupDirectory
                ?? Path.Combine(programData, "WindowsSentinel", "IntegrityBackups");
            _reportDir = config.ReportDirectory
                ?? Path.Combine(programData, "WindowsSentinel", "IncidentReports");
            ...
            AppBaseline baseline, DateTime detectionTime)
        {
            ...
            report.AppendLine($"  {detectionTime:yyyy-MM-dd HH:mm:ss} UTC — Unauthorized replacement detected by Windows Sentinel");
            ...
            report.AppendLine();
            report.AppendLine("═══════════════════════════════════════════════════════════════════");
            report.AppendLine("         Generated by Windows Sentinel Application Integrity Monitor");
            report.AppendLine("═══════════════════════════════════════════════════════════════════");
