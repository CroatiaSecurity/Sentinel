...
/// <summary>
/// Secure Boot &amp; Boot Integrity Monitor (v3.6.0) â€” Detects firmware/boot tampering.
///
/// Monitors:
///   1. Secure Boot state â€” alerts if disabled (bootkits can load)
///   2. Boot configuration (BCD) changes â€” bcdedit tampering
///   3. Driver Signature Enforcement state â€” alerts if disabled
///   4. Test signing mode â€” allows unsigned drivers (rootkit vector)
///   5. Kernel debugging enabled â€” allows kernel-level manipulation
...
/// kernel-level access â€” the most dangerous class of compromise.
///
...
public sealed class SecureBootIntegrityMonitor : BackgroundService
{
    ...
    {
        _logger.LogInformation("[SecureBootIntegrityMonitor] Starting â€” boot integrity monitoring active");
    ...
    private async Task CheckBootIntegrityAsync(CancellationToken ct)
    {
        ...
        if (testSigning == true)
        {
            ...
            {
                // Was disabled, now enabled â€” active tampering
                ...
                if (!_alertedEvents.ContainsKey(dedupeKey))
                {
                    ...
                    await _detectionEngine.EmitAsync(new DetectionEvent
                    {
                        ...
                        Evidence = "Windows Test Signing mode was ENABLED (was previously disabled). " +
                                   "This allows loading unsigned kernel drivers â€” the primary rootkit installation vector.",
                        Reasoning = "Test Signing mode bypasses Driver Signature Enforcement, allowing any " +
                                    ...
                                    "requires admin privileges and a reboot â€” if the user didn't do it, " +
            ...
            {
                // First check â€” test signing was already on
