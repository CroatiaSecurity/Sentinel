...
/// <summary>
/// Microphone Session Monitor â€” Detects unauthorized processes with active audio
...
///   opens the microphone capture device and WRITES audio into it â€” feeding fake
...
///   5. Track session participants over time â€” a NEW participant appearing on a mic
...
///   - User's voice chat app (Discord, Teams, Zoom â€” allowlisted)
///   - Browser-based calls (Chrome, Edge, Firefox â€” allowlisted)
///   - Voice assistants, accessibility tools
///   - The user's own recording software (OBS, Audacity â€” allowlisted)
///
/// Feeds into BehavioralCorrelationEngine composites:
///   - MicSession + Network â†’ "Audio Injection: Mic Feed + Network" (impersonation)
///   - MicSession + ScreenCapture â†’ "Full Surveillance Suite"
///
/// MITRE ATT&amp;CK: T1123 (Audio Capture â€” inverse: audio injection to capture device)
/// </summary>
public sealed class MicSessionMonitor : BackgroundService
{
    ...
    // Track known mic session PIDs â€” used to detect NEW participants
    private readonly ConcurrentDictionary<int, DateTime> _knownMicPids = new();
    ...
    // â”€â”€ Allowlisted processes (legitimate mic users) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ COM interfaces for WASAPI audio session enumeration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private const int eAll = 0;             // Not used â€” we only want capture
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        // Initial delay â€” let system audio stabilize after boot/login
    ...
    private async Task ScanMicSessionsAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            foreach (var pid in sessionPids)
            {
                ...
                // Skip processes with a visible window â€” user is aware of them
                if (ProcessOwnsVisibleWindow(pid)) continue;

                // Skip if it's the foreground app
                if (IsProcessForeground(pid)) continue;

                // Confirmation threshold â€” must persist across scans
                ...
                try
                {
                    ...
                    processAge = DateTime.UtcNow - p.StartTime.ToUniversalTime();
                }
                catch { }
                ...
                _logger.LogWarning(
                    ...
                    "on microphone capture device with no visible window â€” possible audio injection/impersonation",
                    processName, pid);
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Reasoning = "A process with no visible window holding an active audio session on a " +
                               "microphone capture device is a strong indicator of audio injection â€” " +
                ...
                _fusionEngine?.IngestFileActivity(pid, processName,
                    "mic_session_injection", FileActivityKind.Write, DateTime.UtcNow);
            }
        }
        finally
        {
            ...
            foreach (var pid in activeMicPids)
                _knownMicPids.TryAdd(pid, DateTime.UtcNow);
    ...
    // WASAPI COM enumeration â€” get PIDs with active capture sessions
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    /// <summary>
    /// Enumerates all active audio sessions on capture (microphone) devices
    /// and returns the set of PIDs that hold sessions.
    /// Uses WASAPI COM: IMMDeviceEnumerator â†’ IMMDevice â†’ IAudioSessionManager2
    /// â†’ IAudioSessionEnumerator â†’ IAudioSessionControl2 â†’ GetProcessId
    /// </summary>
    private List<int> EnumerateCaptureSessionPids()
    {
        ...
        // S_OK (0) or S_FALSE (1) = already initialized â€” both are fine
            ...
            for (int i = 0; i < deviceCount; i++)
            {
                ...
                try
                {
                    ...
                    for (int j = 0; j < sessionCount; j++)
                    {
                        ...
                        {
                            // Individual session failure â€” continue to next
                        }
                        finally
                        {
                ...
                {
                    // Individual device failure â€” continue to next
                }
                finally
                {
    ...
    // â”€â”€ COM delegate signatures â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Window visibility helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
