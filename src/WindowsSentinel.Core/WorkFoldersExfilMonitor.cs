...
/// <summary>
/// Work Folders Exfiltration Monitor â€” Detects unauthorized activation or abuse of
...
///   T1052     â€” Exfiltration Over Physical Medium (sync to controlled server)
///   T1484.001 â€” Domain Policy Modification: Group Policy Modification
/// </summary>
public sealed class WorkFoldersExfilMonitor : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        // Take baseline â€” is Work Folders already configured?
        TakeBaseline();
        ...
        if (_wasConfiguredAtStartup)
        {
            ...
            // Emit immediate alert â€” this shouldn't be configured on a personal machine
            ...
            if (key != null)
            {
                ...
                if (!string.IsNullOrEmpty(serverUrl) && serverUrl != _baselineServerUrl)
                {
                    ...
                    await EmitDetectionAsync(
                        ...
                        "this is unauthorized and indicates data theft setup â€” either by malware, " +
        ...
        foreach (var path in PolicyRegistryPaths)
        {
            try
            {
                ...
                await EmitDetectionAsync(
                    ...
                    "â€” potentially by ISP-level surveillance, government tools, or malware that " +
