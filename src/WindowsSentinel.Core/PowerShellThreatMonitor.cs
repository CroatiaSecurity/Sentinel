...
/// <summary>
/// PowerShell Threat Monitor â€” Detects malicious PowerShell usage by subscribing to
...
///
/// Detection vectors:
///   1. Encoded commands (-EncodedCommand / -enc) â€” obfuscation technique
...
///   T1562.001 â€” Impair Defenses: Disable or Modify Tools (AMSI bypass)
///   T1027     â€” Obfuscated Files or Information (encoded commands)
///   T1105     â€” Ingress Tool Transfer (download cradles)
/// </summary>
public sealed class PowerShellThreatMonitor : BackgroundService
{
    ...
    // THREAT PATTERNS â€” Organized by severity
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    /// <summary>
    /// Critical patterns â€” immediate kill authorization (AMSI bypass, credential theft).
    ...
    /// <summary>
    /// High-severity patterns â€” strong indicators of malicious activity.
    /// </summary>
    ...
    /// <summary>
    /// Medium-severity patterns â€” suspicious but may have legitimate uses.
    ...
    private async Task AnalyzeScriptBlockAsync(string scriptBlock, string processName, int processId, CancellationToken ct)
    {
        ...
        foreach (var (pattern, description, confidence) in CriticalPatterns)
        {
            if (scriptLower.Contains(pattern))
            {
                ...
                _logger.LogCritical(
                    "POWERSHELL THREAT: CRITICAL â€” {Description} in PID {Pid}",
        ...
        foreach (var (pattern, description, confidence) in HighPatterns)
        {
            if (scriptLower.Contains(pattern))
            {
                ...
                _logger.LogWarning(
                    "POWERSHELL THREAT: HIGH â€” {Description} in PID {Pid}",
                    description, processId);
        ...
        // Check medium-severity patterns (Tier2 â€” log only)
            ...
            if (scriptLower.Contains(pattern))
            {
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Reasoning = "This PowerShell activity matches patterns commonly seen in attacks but may " +
                               "also have legitimate uses. Logged as a corroborating indicator â€” if combined " +
