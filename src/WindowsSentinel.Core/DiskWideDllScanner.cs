...
/// <summary>
/// Disk-Wide DLL Scanner â€” Scans all drives for unsigned/suspicious DLLs that are
...
public sealed class DiskWideDllScanner : BackgroundService
{
    ...
    private DateTime _lastHighRiskScan = DateTime.MinValue;
    private DateTime _lastDriveRootScan = DateTime.MinValue;

    // Hash â†’ (isValid signature, scanTime)
        ...
        try
        {
            ...
            if (!iocMatch && _reputationService != null)
            {
                try
                {
                    ...
                    {
                        // High-confidence clean â€” skip
                        _scannedHashes[hash] = new ScannedDllRecord { IsValid = true, ScannedAt = DateTime.UtcNow };
                        return;
                    }
                }
                catch { /* Network failure â€” continue with local analysis */ }
            }
