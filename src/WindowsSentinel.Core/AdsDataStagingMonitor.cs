...
/// <summary>
/// NTFS Alternate Data Stream (ADS) Staging Monitor â€” Detects processes writing
...
///   visible file accounts for it â€” the classic "invisible disk fill" pattern.
///
/// Detection approach:
...
///   3. Flag any ADS larger than a threshold (10MB) â€” legitimate ADS are tiny
///      (Zone.Identifier is ~26 bytes, thumbnails are ~100KB max)
///   4. Track cumulative ADS writes per process â€” large total = staging
///
/// Legitimate ADS uses (allowlisted):
///   - Zone.Identifier (mark-of-the-web) â€” always tiny
///   - SmartScreen â€” tiny
///   - Thumbnails â€” small
///   - SummaryInformation â€” small
///
/// MITRE ATT&amp;CK: T1564.004 â€” Hide Artifacts: NTFS File Attributes
/// </summary>
public sealed class AdsDataStagingMonitor : BackgroundService
{
    ...
    private readonly ConcurrentDictionary<string, DateTime> _alertedPaths = new();
    ...
    private async Task ScanForSuspiciousAdsAsync(CancellationToken ct)
    {
        ...
        foreach (var root in ScanRoots)
        {
            ...
            try
            {
                // Scan files in this directory (non-recursive for performance â€” only top 2 levels)
                foreach (var file in EnumerateFilesLimited(root, maxDepth: 2))
                {
                    ...
                    foreach (var (streamName, streamSize) in streams)
                    {
                        ...
                        // This is a large, non-standard ADS â€” suspicious
                        var fullStreamPath = $"{file}:{cleanName}";
                        if (_alertedPaths.ContainsKey(fullStreamPath)) continue;

                        _alertedPaths[fullStreamPath] = DateTime.UtcNow;
                        ...
                        _logger.LogCritical(
                            "ADS Staging: Large alternate data stream detected â€” {File}:{Stream} ({Size:F1} MB)",
                        ...
                        await _detectionEngine.EmitAsync(new DetectionEvent
                        {
                            ...
                            Evidence = $"File '{file}' has a large alternate data stream '{cleanName}' " +
                                      ...
                                      $"Large ADS are invisible to Explorer and standard file listings â€” " +
                            ...
                            Reasoning = "NTFS Alternate Data Streams are invisible to normal file browsing. " +
                                       ...
                                       "data staging for exfiltration â€” the attacker hides stolen data in " +
