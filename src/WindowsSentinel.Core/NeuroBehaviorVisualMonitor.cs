...
/// <summary>
/// NeuroBehavior Visual Monitor â€” Detects visual/input manipulation attacks.
///
/// Ported from Antivirus.ps1 Invoke-NeuroBehaviorMonitor. Detects:
///   1. Focus abuse â€” process stealing focus rapidly (>8 times in 10s)
///   2. Flash stimulus â€” rapid brightness changes (strobing/flashing)
///   3. Topmost abuse â€” non-allowlisted process forcing WS_EX_TOPMOST
///   4. Cursor jitter â€” rapid programmatic cursor movement
///   5. Color distortion/inversion â€” screen colors being manipulated
///
...
///
/// IMPORTANT: The Pre-Kill Validation Gate (v2.2.0) provides additional safety â€”
...
public sealed class NeuroBehaviorVisualMonitor : BackgroundService
{
    ...
    // Scan interval â€” 1 second (matches original PS1 behavior)
    private static readonly TimeSpan ScanInterval = TimeSpan.FromSeconds(1);

    // â”€â”€ Focus abuse tracking â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    private readonly ConcurrentDictionary<int, FocusEntry> _focusHistory = new();

    // â”€â”€ Flash/brightness tracking â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Cursor jitter tracking â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    private Point _lastCursorPos;
    private DateTime _cursorFirstSeen = DateTime.MinValue;
    ...
    // â”€â”€ Color distortion tracking â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Deduplication (report each process only once per type) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    private readonly ConcurrentDictionary<string, DateTime> _reportedItems = new();
    private static readonly TimeSpan ReportCooldown = TimeSpan.FromMinutes(5);

    // â”€â”€ Topmost allowlist (legitimate always-on-top processes) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Win32 imports â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        // Initial delay â€” let desktop stabilize after login
    ...
    private async Task ScanAsync(CancellationToken ct)
    {
        ...
        // â”€â”€ Screen sample (64x64 from top-left corner) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // â”€â”€ 1. Focus abuse detection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        await DetectFocusAbuse(fgPid, processName, ct);

        // â”€â”€ 2. Flash stimulus detection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if (screenSampled)
            await DetectFlashStimulus(brightness, fgPid, processName, ct);

        // â”€â”€ 3. Topmost abuse detection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        await DetectTopmostAbuse(hWnd, fgPid, processName, ct);

        // â”€â”€ 4. Cursor jitter detection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        await DetectCursorJitter(fgPid, processName, ct);

        // â”€â”€ 5. Color distortion/inversion detection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if (screenSampled)
    ...
    // programmatic focus manipulation â€” used to force user attention or
    ...
    private async Task DetectFocusAbuse(int pid, string processName, CancellationToken ct)
    {
        ...
        var elapsed = (DateTime.UtcNow - entry.FirstSeen).TotalSeconds;
        ...
        if (elapsed < 10 && entry.Count > 8)
        {
            ...
            {
                _logger.LogWarning(
                    "NeuroBehaviorVisual: Focus abuse by {Process} (PID {Pid}) â€” {Count} focus steals in {Elapsed:F1}s",
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Reasoning = "Rapid programmatic focus stealing is used to force user attention, " +
                               ...
                               "This is a Tier2 advisory signal â€” it feeds composite correlation but " +
                ...
                _fusionEngine?.IngestFileActivity(pid, processName,
                    "neuro_focus_abuse", FileActivityKind.Read, DateTime.UtcNow);
            }
    ...
    private async Task DetectFlashStimulus(long brightness, int pid, string processName, CancellationToken ct)
    {
        ...
        if (_flashScore >= FlashThreshold)
        {
            ...
            {
                _logger.LogWarning(
                    "NeuroBehaviorVisual: Flash stimulus detected from {Process} (PID {Pid}) â€” score {Score}",
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Reasoning = "Rapid screen brightness changes (strobing/flashing) can cause " +
                               ...
                               "influence technique. This is a Tier2 advisory signal â€” video content " +
                ...
                _fusionEngine?.IngestFileActivity(pid, processName,
                    "neuro_flash_stimulus", FileActivityKind.Read, DateTime.UtcNow);
            }
        ...
        if (ShouldReport($"NBM_Topmost:{processName}"))
        {
            ...
            await _detectionEngine.EmitAsync(new DetectionEvent
            {
                ...
                Reasoning = "A non-allowlisted process forcing itself to always-on-top can be used " +
                           ...
                           "or forcing user attention. This is a Tier2 advisory signal â€” some " +
            ...
            _fusionEngine?.IngestFileActivity(pid, processName,
                "neuro_topmost_abuse", FileActivityKind.Read, DateTime.UtcNow);
        }
    }
    ...
    // cursor manipulation â€” used to disorient or prevent user interaction.
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    private async Task DetectCursorJitter(int pid, string processName, CancellationToken ct)
    {
        ...
        if (_cursorFirstSeen == DateTime.MinValue)
        {
            _cursorFirstSeen = DateTime.UtcNow;
            return;
        }

        var elapsed = (DateTime.UtcNow - _cursorFirstSeen).TotalSeconds;
        ...
        if (elapsed < 10 && _cursorJitterCount > CursorJitterThreshold)
        {
            ...
            {
                _logger.LogWarning(
                    "NeuroBehaviorVisual: Cursor jitter abuse from {Process} (PID {Pid}) â€” {Count} jumps in {Elapsed:F1}s",
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Reasoning = "Rapid programmatic cursor movement can be used to disorient the user, " +
                               ...
                               "tool actively controlling the mouse. This is a Tier2 advisory signal â€” " +
                ...
                _fusionEngine?.IngestFileActivity(pid, processName,
                    "neuro_cursor_jitter", FileActivityKind.Read, DateTime.UtcNow);
            }
    ...
    // manipulation â€” used for disorientation or subliminal influence.
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        ...
        int pid, string processName, CancellationToken ct)
    {
        ...
        // Check for color inversion (current â‰ˆ inverse of previous)
            ...
            if (ShouldReport($"NBM_Color:{processName}"))
            {
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Evidence = $"Screen color inversion detected while '{processName}' (PID {pid}) " +
                              $"is in foreground. Current RGB ({avgR:F0},{avgG:F0},{avgB:F0}) â‰ˆ " +
                    ...
                    Reasoning = "Screen color inversion is a visual manipulation technique that can " +
                               ...
                               "advisory signal â€” accessibility features (high contrast) can trigger " +
                ...
                _fusionEngine?.IngestFileActivity(pid, processName,
                    "neuro_color_inversion", FileActivityKind.Read, DateTime.UtcNow);
            }
        }
        else
        {
            ...
            if (_distortScore >= DistortThreshold)
            {
                ...
                {
                    _logger.LogWarning(
                        "NeuroBehaviorVisual: Screen distortion from {Process} (PID {Pid}) â€” score {Score}",
                    ...
                    await _detectionEngine.EmitAsync(new DetectionEvent
                    {
                        ...
                        Reasoning = "Rapid color shifts without inversion indicate screen manipulation " +
                                   "â€” potentially subliminal color cycling or visual disorientation. " +
                                   "This is a Tier2 advisory signal â€” video content and games cause " +
                    ...
                    _fusionEngine?.IngestFileActivity(pid, processName,
                        "neuro_color_distortion", FileActivityKind.Read, DateTime.UtcNow);
                }
    ...
    private bool ShouldReport(string key)
    {
        ...
        {
            if (DateTime.UtcNow - lastReport < ReportCooldown)
                return false;
        }
        _reportedItems[key] = DateTime.UtcNow;
        return true;
    }

    private void CleanupReportedItems()
    {
        var cutoff = DateTime.UtcNow - ReportCooldown;
        ...
        {
            if ((DateTime.UtcNow - kv.Value.FirstSeen).TotalMinutes > 5)
