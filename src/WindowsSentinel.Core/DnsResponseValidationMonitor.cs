...
/// <summary>
/// DNS Response Validation Monitor (v3.6.0) â€” Detects DNS poisoning and hijacking.
///
...
///
/// Does NOT require external API calls for validation â€” uses hardcoded CIDR ranges
...
public sealed class DnsResponseValidationMonitor : BackgroundService
{
    ...
    private static readonly CanaryDomain[] CanaryDomains =
    {
        ...
        {
            "20.0.0.0/8",       // Microsoft Azure (broad â€” MS uses many /8s)
    ...
    {
        _logger.LogInformation("[DnsResponseValidationMonitor] Starting â€” DNS response integrity monitoring active");
    ...
    private async Task ValidateDnsResponsesAsync(CancellationToken ct)
    {
        foreach (var canary in CanaryDomains)
        {
            ...
            {
                // DNS resolution failed â€” could be network issue, not necessarily an attack
            }
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
    ...
    private async Task<string?> CrossValidateAsync(string domain, CancellationToken ct)
    {
        ...
        // For simplicity, we do a second resolution â€” if the system DNS is poisoned,
