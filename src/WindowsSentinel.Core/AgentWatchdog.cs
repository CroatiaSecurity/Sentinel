...
{
    /// <summary>
    /// Service-side watchdog that keeps WindowsSentinel.Agent.exe alive in the user's
    /// interactive session.
    ///
    ...
    public sealed class AgentWatchdog : BackgroundService
    {
        ...
        private const string AgentProcessName = "WindowsSentinel.Agent";
        private const string AgentExeName = "WindowsSentinel.Agent.exe";
        ...
        private async Task CheckAgentAsync(CancellationToken ct)
        {
            ...
            if (_killCount >= KillThresholdForAlert && !_alertFired)
            {
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Evidence = $"WindowsSentinel.Agent.exe has been absent {_killCount} times in the last " +
