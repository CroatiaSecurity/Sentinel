...
/// <summary>
/// Public IP Baseline Monitor (v3.6.0) â€” Detects unexpected public IP changes.
///
...
///
/// Privacy: Only checks the IP â€” does NOT send any system information externally.
...
public sealed class PublicIpMonitor : BackgroundService
{
    ...
    private DateTime _baselineCapturedAt;
        ...
        ILogger<PublicIpMonitor> logger)
    {
        ...
        _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("WindowsSentinel/5.1.0");
    }

    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        _logger.LogInformation("[PublicIpMonitor] Starting â€” public IP baseline monitoring active");
        ...
        if (baseline != null)
        {
            ...
            _baselineCapturedAt = DateTime.UtcNow;
        ...
        {
            _logger.LogWarning("[PublicIpMonitor] Could not capture baseline â€” will retry");
        }
    ...
    private async Task CheckPublicIpAsync(CancellationToken ct)
    {
        ...
        if (!string.Equals(_baselineIp, current.Ip, StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (countryChanged)
            {
                ...
                severity = "CRITICAL â€” Country changed (possible VPN hijack or BGP manipulation)";
            }
            else if (asnChanged)
            {
                confidence = 0.82;
                tier = DetectionTier.Tier1Behavioral;
                severity = "HIGH â€” ASN changed (traffic routed through different provider)";
            }
            else
            {
                severity = "MEDIUM â€” IP changed within same ASN (possible ISP reassignment or proxy)";
            }
            ...
            if (!_alertedEvents.ContainsKey(dedupeKey))
            {
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Reasoning = "Public IP changes without user-initiated network events indicate: " +
                                ...
                                "Country/ASN changes are highest severity â€” legitimate ISP reassignment " +
