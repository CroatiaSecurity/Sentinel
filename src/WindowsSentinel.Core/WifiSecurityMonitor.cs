...
/// <summary>
/// Wi-Fi Security Monitor (v3.6.0) â€” Detects Wi-Fi attacks and insecure connections.
///
/// Monitors the wireless network state for:
///   1. Connection to open (unencrypted) networks â€” traffic visible to anyone nearby
///   2. Downgrade from WPA3/WPA2 to WEP or Open â€” possible deauth + evil twin
///   3. SSID changes without user action â€” possible evil twin AP
///   4. Frequent disconnections â€” possible deauthentication attack
...
public sealed class WifiSecurityMonitor : BackgroundService
{
    ...
    private readonly Queue<DateTime> _disconnectTimes = new();
    ...
    {
        _logger.LogInformation("[WifiSecurityMonitor] Starting â€” Wi-Fi security monitoring active");
    ...
    private async Task CheckWifiStateAsync(CancellationToken ct)
    {
        ...
        {
            // Just disconnected
            _disconnectTimes.Enqueue(DateTime.UtcNow);
            _wasConnected = false;

            // Prune old disconnect events
            while (_disconnectTimes.Count > 0 &&
                   DateTime.UtcNow - _disconnectTimes.Peek() > DeauthTimeWindow)
            ...
            if (!_alertedEvents.ContainsKey(dedupeKey))
            {
                ...
                await _detectionEngine.EmitAsync(new DetectionEvent
                {
                    ...
                    Reasoning = "Open Wi-Fi networks provide zero encryption. Any device within range " +
                                ...
                                "the typical state after a successful evil twin attack â€” the rogue AP " +
        ...
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        // CHECK 2: Encryption downgrade (WPA2/3 â†’ WEP or Open)
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        ...
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        // CHECK 3: BSSID changed for same SSID (different AP â€” possible evil twin)
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
