...
namespace WindowsSentinel.Core
{
    ...
    public sealed class ShellWatchdog : BackgroundService
    {
        ...
        {
            _logger.LogInformation("[ShellWatchdog] Started");
        ...
        private async Task TryRestartExplorerAsync(CancellationToken ct)
        {
            ...
            try
            {
                ...
                // Start explorer.exe — Windows will auto-initialize it as the shell
                ...
                if (proc != null)
                {
                    _logger.LogInformation("[ShellWatchdog] Restarted explorer.exe as PID {Pid}", proc.Id);
                    await _eventLogger.LogEventAsync("shell_restart", new
                    {
                        NewPid = proc.Id,
            ...
            {
                _logger.LogError(ex, "[ShellWatchdog] Failed to restart explorer.exe");
            }
        }
