...
/// <summary>
/// Runtime Module Integrity Monitor â€” Validates ALL loaded modules across ALL running
...
///   - Tier A (critical processes): scanned every 30s â€” lsass, csrss, svchost, winlogon, etc.
///   - Tier B (high-value targets): scanned every 60s â€” browsers, office, comms, Sentinel
...
///
///   1. Runtime DLL Injection â€” new unsigned/suspicious module appears after baseline
///   2. DLL Replacement â€” loaded module's on-disk hash changes between scans
///   3. Phantom Module â€” module's backing file deleted from disk (dropper pattern)
///   4. Signature Invalidation â€” previously-signed module now fails validation
///
/// Composite correlation (via BehavioralCorrelationEngine):
///   - Module injection + Network â†’ "Injected C2 Implant" (kill-authorized)
///   - Module injection + Clipboard â†’ "Clipboard Exfil via Injection" (kill-authorized)
///   - Module injection + LSASS signal â†’ "Credential Theft Tool Loaded"
///   - Module injection + Privilege escalation â†’ "Post-Exploit Payload"
///
/// MITRE ATT&CK:
///   T1055 â€” Process Injection (all sub-techniques)
///   T1574 â€” Hijack Execution Flow
///   T1129 â€” Shared Modules
/// </summary>
public sealed class RuntimeModuleIntegrityMonitor : BackgroundService
{
    ...
    // Per-process module baselines: PID â†’ baseline
    ...
    // Scan timing
    private DateTime _lastTierAScan = DateTime.MinValue;
    private DateTime _lastTierBScan = DateTime.MinValue;
    private DateTime _lastTierCScan = DateTime.MinValue;
    ...
    private static readonly HashSet<string> KnownLateLoadModules = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        // Diagnostics (legitimate â€” dbghelp flagged separately by LsassDumpCanaryMonitor)
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        // Main scan loop â€” runs every 10s, but each tier has its own interval
    ...
    private async Task ScanSingleProcessAsync(Process proc, string tier, CancellationToken ct)
    {
        ...
        {
            // Existing process â€” check for deviations
            await CheckProcessDeviationsAsync(proc, baseline, tier, ct);
        }
        else
        {
            // New process â€” establish baseline
        ...
        foreach (ProcessModule module in modules)
        {
            try
            {
                ...
                baseline.Modules[path.ToLowerInvariant()] = new ModuleRecord
                {
                    ...
                    Hash = null, // Lazy â€” only compute on deviation check
                    IsSigned = null, // Lazy
                    Publisher = null,
        ...
        string tier, CancellationToken ct)
    {
        ...
        foreach (ProcessModule module in currentModules)
        {
            ...
            try
            {
                ...
                {
                    // NEW module not in baseline â€” possible injection
        ...
        ProcessModuleBaseline baseline, string tier, CancellationToken ct)
    {
        ...
        if (DateTime.UtcNow - baseline.CapturedAt < BaselineGracePeriod) return;
        ...
        // Full analysis â€” compute hash and check signature
        ...
        {
            // Trusted publisher â€” add to baseline silently
            baseline.Modules[pathLower] = new ModuleRecord
            {
                Path = path, ModuleName = moduleName, Hash = hash,
                IsSigned = true, Publisher = publisher, FirstSeen = DateTime.UtcNow
            };
            return;
        }
        ...
        if (score < 35)
        {
            ...
            {
                Path = path, ModuleName = moduleName, Hash = hash,
                IsSigned = isSigned, Publisher = publisher, FirstSeen = DateTime.UtcNow
            };
            return;
        }
        ...
        // Feed telemetry fusion â€” enables correlation with network/clipboard/LSASS signals
        _fusionEngine?.IngestFileActivity(proc.Id, proc.ProcessName,
            path, FileActivityKind.Read, DateTime.UtcNow);
    ...
    private async Task EmitPhantomModuleAsync(Process proc, ModuleRecord phantom, CancellationToken ct)
    {
        ...
        _logger.LogWarning(
            "[PHANTOM-MODULE] '{Module}' in '{Process}' (PID {Pid}) â€” file deleted from disk",
        ...
        _fusionEngine?.IngestFileActivity(proc.Id, proc.ProcessName,
            phantom.Path, FileActivityKind.Delete, DateTime.UtcNow);
    }
