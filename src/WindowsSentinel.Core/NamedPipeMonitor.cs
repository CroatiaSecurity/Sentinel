...
///
/// Named pipes are a critical blind spot for EDRs â€” many C2 frameworks
...
public sealed class NamedPipeMonitor : IMonitor
{
    ...
    private readonly ConcurrentDictionary<string, DateTime> _alertedPipes = new();
    private static readonly TimeSpan AlertDedupeWindow = TimeSpan.FromMinutes(5);

    // â”€â”€ Known malicious pipe name patterns â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private async Task ScanNamedPipesAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var pipePath in pipes)
        {
            ...
            if (_alertedPipes.TryGetValue(dedupeKey, out var lastAlert) &&
                DateTime.UtcNow - lastAlert < AlertDedupeWindow)
            {
                continue;
            }

            _alertedPipes[dedupeKey] = DateTime.UtcNow;
    ...
    private static int TryGetPipeOwnerPid(string pipePath)
    {
        ...
        {
            // Access denied or pipe closed â€” expected for many system pipes
        }
