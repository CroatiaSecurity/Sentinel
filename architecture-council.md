...
**Status:** Historical architecture rationale. **Runtime inventory and versions are authoritative in `design.md` (v2.2.4).** This document explains the President/Council mental model; do not treat monitor tables here as the live registration list.

**Last updated note:** v2.2.4 — generic CVE-class monitors live in `design.md`. EDR-killer process names are **not** President's Law (LogOnly observe fuel). All monitor classes are `namespace Sentinel.Core`.
