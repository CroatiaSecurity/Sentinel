...
- **Module identity unload (v2.2.5)** — every mapped PE is path+signer checked. Foreign modules (Temp/Downloads drops, unsigned sideload plants, random folders) are FreeLibrary-APC unloaded immediately. Chromium loading Edge DLLs is not treated as injection. Unbacked RWX with an MZ header or compact shellcode has execute stripped.
...
Download **`SentinelSetup-2.2.6.exe`** from [GitHub Releases](https://github.com/CroatiaSecurity/Sentinel/releases) (or `releases/2.2.6/` after a local build) and run it as Administrator.
