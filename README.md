...
- **Observe by default** (`ObserveUntilChain: true`): every monitor logs to `events.jsonl`; **no kill / quarantine / isolate / host rewrite / toast / evidence pack** on single-signal *noise*. **v2.5.2/2.5.3:** attributed attack-class hits (tailcat mesh, webhook stealers, potato/LPE tools, kernel-EoP loaders, ClickFix, Hell's Gate, unmapped thread, CS pipes, classic RAT ports, ngrok/chisel, impostor AMSI) solo-confirm and kill. **v2.5.4:** cross-PID staged attacks (loader spawns a fresh child per malicious action) accumulate signals in a shared ancestry-root buffer — fresh PIDs no longer reset the chain. Real Discord/Chrome/games/official Tailscale (install path or Authenticode) still do not emit those rules. `discord.exe` in Temp is not Discord.
...
8. **Settings UI** — Tray **Settings** (double-click) opens the **embedded web dashboard** in a native window: Overview, Events, System Scan, Quarantine, Report to Police, Safety, **Ops Metrics**, Tools, About. No external browser needed. No ActiveResponse toggle in the agent (service-only).
