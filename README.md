...
### Product posture (v1.8.8 — observe-until-chain + Tier1 kill-grade only)
...
- **Weak / non-terminal signals alone** — Shell using SSH, Downloads network, SeImpersonate alone, System32 redistributable writes (DirectX/GPU), netsh noise: **logged**, not killed. Destructive response requires a multi-signal chain to C2/exfil/token/shell/cred-dump/BYOVD (`ObserveUntilChain`, v1.8.8).
...
Download **`SentinelSetup-1.8.8.exe`** from [GitHub Releases](https://github.com/CroatiaSecurity/Sentinel/releases) (or `releases/1.8.8/` after a local build) and run it as Administrator.
